...
def upgrade():
    ...
    if not result.fetchone():
        ...
        if not owner_result.fetchone():
            op.add_column(
                "assistant_profiles", sa.Column("owner_id", sa.String(), nullable=True)
            )
            # Update existing assistant_profiles to set owner_id from user_id
            op.execute(
                "UPDATE assistant_profiles SET owner_id = user_id WHERE owner_id IS NULL"
            )
            op.alter_column("assistant_profiles", "owner_id", nullable=False)

        # Check for is_public column
        public_result = connection.execute(
            sa.text(
                """
            SELECT column_name
            FROM information_schema.columns
        ...
        op.create_table(
            ...
            sa.ForeignKeyConstraint(
                ["assistant_id"], ["assistant_profiles.id"], ondelete="CASCADE"
            ),
            sa.UniqueConstraint(
                "assistant_id", "user_id", name="uq_assistant_user_permission"
            ),
        )

        # Create indexes
        op.create_index(
            "ix_assistant_permissions_user", "assistant_permissions", ["user_id"]
        )
        op.create_index(
            "ix_assistant_permissions_assistant",
            "assistant_permissions",
            ["assistant_id"],
        )
        op.create_index(
            "ix_assistant_profiles_owner", "assistant_profiles", ["owner_id"]
        )
...
def downgrade():
    ...
    op.drop_index(
        "ix_assistant_permissions_assistant", table_name="assistant_permissions"
    )
