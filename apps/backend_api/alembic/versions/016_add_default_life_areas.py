...
def upgrade():
    ...
    result = connection.execute(
        sa.text("SELECT COUNT(*) FROM users WHERE uid = 'system'")
    )
    if result.scalar() == 0:
        connection.execute(
            sa.text(
    ...
    result = connection.execute(
        sa.text("SELECT COUNT(*) FROM life_areas WHERE user_id = 'system'")
    )
    if result.scalar() == 0:
        ...
        life_areas_data = [
            (
                "Health & Fitness",
                "Physical health, exercise, nutrition, and overall well-being",
                "#FF6B6B",
                "favorite",
                '["health", "fitness", "exercise", "nutrition", "wellness", "medical", "sport"]',
                1,
            ),
            (
                "Career & Work",
                "Professional development, job satisfaction, and career growth",
                "#4ECDC4",
                "work",
                '["career", "work", "job", "professional", "business", "office", "employment"]',
                2,
            ),
            (
                "Relationships",
                "Family, friends, romantic relationships, and social connections",
                "#FFE66D",
                "family_restroom",
                '["family", "friends", "love", "social", "relationships", "partner", "community"]',
                3,
            ),
            (
                "Personal Growth",
                "Learning, self-improvement, education, and skill development",
                "#A8E6CF",
                "school",
                '["growth", "learning", "education", "skills", "development", "improvement", "study"]',
                4,
            ),
            (
                "Finance",
                "Money management, savings, investments, and financial security",
                "#C7CEEA",
                "attach_money",
                '["money", "finance", "budget", "savings", "investment", "wealth", "income"]',
                5,
            ),
            (
                "Spirituality",
                "Spiritual practice, mindfulness, meditation, and inner peace",
                "#FFDAB9",
                "self_improvement",
                '["spiritual", "meditation", "mindfulness", "faith", "religion", "inner peace", "soul"]',
                6,
            ),
            (
                "Fun & Recreation",
                "Hobbies, entertainment, leisure activities, and enjoyment",
                "#B4A7D6",
                "music_note",
                '["fun", "recreation", "hobbies", "entertainment", "leisure", "play", "relaxation"]',
                7,
            ),
            (
                "Environment",
                "Living space, home environment, and physical surroundings",
                "#D4A5A5",
                "home",
                '["home", "environment", "living space", "organization", "comfort", "surroundings"]',
                8,
            ),
        ]
