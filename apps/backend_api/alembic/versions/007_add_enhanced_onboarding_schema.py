...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "color", sa.String(7), nullable=False, server_default="#6366f1"
        ),  # Indigo default
    ...
    op.create_index(
        "ix_custom_life_areas_priority",
        "custom_life_areas",
        ["user_id", "priority_order"],
    )
    ...
    op.create_table(
        ...
        ),  # 'step_start', 'step_complete', 'abandon'
        sa.Column(
            "step_name", sa.String(50), nullable=True
        ),  # 'welcome', 'ai_config', 'personal_config'
    ...
    op.create_index(
        "ix_onboarding_analytics_user_id", "onboarding_analytics", ["user_id"]
    )
    op.create_index(
        "ix_onboarding_analytics_event",
        "onboarding_analytics",
    ...
    op.execute(
        "UPDATE onboarding_states SET flow_version = 'v1' WHERE flow_version IS NULL"
    )
