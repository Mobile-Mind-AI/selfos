...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_projects_life_area_created",
        "projects",
        ["life_area_id", sa.text("created_at DESC")],
        unique=False,
    )
    op.create_index(
        "ix_projects_user_priority", "projects", ["user_id", "priority"], unique=False
    )

    # Add project_id to goals table
    op.add_column("goals", sa.Column("project_id", sa.Integer(), nullable=True))
    op.create_foreign_key(
        "fk_goals_project", "goals", "projects", ["project_id"], ["id"]
    )
    op.create_index(
        "ix_goals_project_created",
        "goals",
    ...
    op.create_foreign_key(
        "fk_tasks_project", "tasks", "projects", ["project_id"], ["id"]
    )
    op.create_index(
    ...
    op.add_column(
        "media_attachments", sa.Column("project_id", sa.Integer(), nullable=True)
    )
    op.create_foreign_key(
        "fk_media_project", "media_attachments", "projects", ["project_id"], ["id"]
    )
    op.create_index(
        "ix_media_project",
        "media_attachments",
