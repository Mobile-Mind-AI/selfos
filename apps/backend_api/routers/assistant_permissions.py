...
async def get_user_assistants(
    ...
    assistants = await PermissionService.get_user_assistants(current_user["uid"], db)
...
async def share_assistant(
    ...
    try:
        ...
        return ShareAssistantResponse(
            success=True, message="Assistant shared successfully"
        )
...
async def get_assistant_permissions(
    ...
    permissions = await PermissionService.get_assistant_permissions(assistant_id, db)
