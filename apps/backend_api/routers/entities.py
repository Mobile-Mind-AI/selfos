...
from schemas import EntityRelationshipSchema, EntitySchema, EntityTypeSchema
...
def get_entities(
    ...
    query = query.order_by(
        Entity.importance.desc(), Entity.last_interaction.desc().nullslast()
    )
...
def get_entity(
    ...
    entity = (
        db.query(Entity)
        .filter(Entity.id == entity_id, Entity.user_id == current_user["uid"])
        .first()
    )
...
def associate_entity_with_goal(
    ...
    goal = (
        db.query(Goal)
        .filter(Goal.id == goal_id, Goal.user_id == current_user["uid"])
        .first()
    )
    ...
    existing = (
        db.query(GoalEntity)
        .filter(GoalEntity.goal_id == goal_id, GoalEntity.entity_id == entity_id)
        .first()
    )
...
def associate_entity_with_project(
    ...
    project = (
        db.query(Project)
        .filter(Project.id == project_id, Project.user_id == current_user["uid"])
        .first()
    )
    ...
    existing = (
        ...
        .filter(
            ProjectEntity.project_id == project_id, ProjectEntity.entity_id == entity_id
        )
        .first()
    )
...
def associate_entity_with_task(
    ...
    task = (
        db.query(Task)
        .filter(Task.id == task_id, Task.user_id == current_user["uid"])
        .first()
    )
    ...
    existing = (
        db.query(TaskEntity)
        .filter(TaskEntity.task_id == task_id, TaskEntity.entity_id == entity_id)
        .first()
    )
...
def get_important_entities(
    ...
    for entity in entities:
        # Count associations
        goal_count = (
            db.query(GoalEntity).filter(GoalEntity.entity_id == entity.id).count()
        )

        project_count = (
            db.query(ProjectEntity).filter(ProjectEntity.entity_id == entity.id).count()
        )

        task_count = (
            db.query(TaskEntity).filter(TaskEntity.entity_id == entity.id).count()
        )
