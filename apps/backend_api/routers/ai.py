    ...
    if ai_orchestrator is None:
        try:
            ...
            if not ai_engine_path:
                raise Exception(
                    f"Could not find ai_engine directory. Tried: {possible_paths}"
                )

            # Add libs path first for prompts
            libs_path = os.path.join(
                os.path.dirname(os.path.dirname(ai_engine_path)), "libs"
            )
            ...
            orchestrator_spec = importlib.util.spec_from_file_location(
                "ai_orchestrator", orchestrator_path
            )
...
@router.post(
    "/decompose-goal", response_model=GoalDecompositionResponse, status_code=200
)
    ...
    try:
        ...
        if memory_service:
            try:
                ...
                logger.info(
                    f"Stored goal decomposition in memory for user {current_user['uid']}"
                )
            except Exception as e:
...
async def chat(
    request: ChatRequest, current_user: UserSchema = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if memory_service:
            try:
                ...
                logger.info(
                    f"Stored conversation in memory for user {current_user['uid']}"
                )
            except Exception as e:
...
async def get_memory_stats(current_user: UserSchema = Depends(get_current_user)):
    """
