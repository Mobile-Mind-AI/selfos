...
@router.post("/habits/{habit_id}/complete", response_model=schemas.HabitCompletion, status_code=201)
def complete_habit(
    habit_id: int,
    ...
    completion_record = habit_service.complete_habit(db, current_user["uid"], habit_id, completion)
    if not completion_record:
        raise HTTPException(status_code=404, detail="Active habit not found")
    return completion_record


@router.get("/habits/{habit_id}/completions", response_model=List[schemas.HabitCompletion])
def get_habit_completions(
    habit_id: int,
    start_date: Optional[date] = Query(None, description="Start date for completion range"),
...
def get_habit_progress(
    habit_id: int,
    target_date: Optional[date] = Query(None, description="Date to calculate progress for (defaults to today)"),
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user)
):
    """Get progress for a habit for the current period."""
    progress = habit_service.get_habit_progress(db, current_user["uid"], habit_id, target_date)
    if not progress:
        raise HTTPException(status_code=404, detail="Habit not found")
    return progress


@router.put("/habits/{habit_id}/completions/{completion_id}", response_model=schemas.HabitCompletion)
...
@router.delete("/habits/{habit_id}/completions/{completion_id}", status_code=status.HTTP_204_NO_CONTENT)
