...
def get_current_assistant_profile(
    ...
    if not profile:
        ...
        profile = (
            db.query(models.AssistantProfile)
            .filter(models.AssistantProfile.user_id == current_user["uid"])
            .first()
        )
...
def update_assistant_profile(
    ...
    if not profile:
        ...
        profile = models.AssistantProfile(user_id=current_user["uid"], is_default=True)
        db.add(profile)
...
def create_assistant_profile(
    ...
    # Create new profile
    profile = models.AssistantProfile(
        user_id=current_user["uid"], **profile_data.model_dump()
    )
...
def get_assistant_profile(
    ...
    if not profile:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Assistant profile not found"
        )
...
def delete_assistant_profile(
    ...
    return {
        "message": "Assistant profile deleted successfully",
