...
def get_journal_entries(
    ...
    start_date: Optional[datetime] = Query(None, description="Filter entries created after this date"),
    end_date: Optional[datetime] = Query(None, description="Filter entries created before this date"),
...
def get_journal_entries_for_parent(
    ...
    if parent_type not in ['project', 'goal', 'task']:
        raise HTTPException(
            status_code=400, 
            detail="parent_type must be 'project', 'goal', or 'task'"
        )
