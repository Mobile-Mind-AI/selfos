...
def get_feedback_logs(
    ...
    feedback_logs = (
        query.order_by(desc(FeedbackLog.created_at)).offset(offset).limit(limit).all()
    )
...
def get_feedback_log(
    ...
    feedback_log = (
        ...
        .filter(FeedbackLog.id == feedback_id, FeedbackLog.user_id == user_id)
        .first()
    )

    if not feedback_log:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Feedback log not found"
        )
...
def create_feedback_log(
    ...
    feedback_log = FeedbackLog(
        user_id=user_id, **feedback_data.model_dump(exclude_unset=True)
    )
...
def get_feedback_summary(
    ...
    base_query = db.query(FeedbackLog).filter(
        FeedbackLog.user_id == user_id, FeedbackLog.created_at >= start_date
    )

    # Get total counts by feedback type
    feedback_counts = (
        db.query(FeedbackLog.feedback_type, func.count(FeedbackLog.id).label("count"))
        .filter(FeedbackLog.user_id == user_id, FeedbackLog.created_at >= start_date)
    ...
    avg_score_result = (
        db.query(func.avg(FeedbackLog.feedback_value))
        .filter(
    ...
    context_counts = (
        db.query(FeedbackLog.context_type, func.count(FeedbackLog.id).label("count"))
        .filter(FeedbackLog.user_id == user_id, FeedbackLog.created_at >= start_date)
        .group_by(FeedbackLog.context_type)
        .all()
    )
        ...
        "context_types": [
            {
                "value": "task",
                "label": "Task",
                "description": "Feedback related to task interactions",
            },
            {
                "value": "goal",
                "label": "Goal",
                "description": "Feedback related to goal interactions",
            },
            {
                "value": "plan",
                "label": "Plan",
                "description": "Feedback on planning and suggestions",
            },
            {
                "value": "suggestion",
                "label": "AI Suggestion",
                "description": "Feedback on AI-generated suggestions",
            },
            {
                "value": "ui_interaction",
                "label": "UI Interaction",
                "description": "Feedback on user interface elements",
            },
            {
                "value": "search",
                "label": "Search",
                "description": "Feedback on search functionality",
            },
            {
                "value": "notification",
                "label": "Notification",
                "description": "Feedback on notifications",
            },
            {
                "value": "report",
                "label": "Report",
                "description": "Feedback on reports and analytics",
            },
            {
                "value": "import",
                "label": "Data Import",
                "description": "Feedback on data import processes",
            },
            {
                "value": "export",
                "label": "Data Export",
                "description": "Feedback on data export processes",
            },
            {
                "value": "general",
                "label": "General",
                "description": "General application feedback",
            },
        ]
    }
...
def delete_bulk_feedback_logs(
    ...
    feedback_logs = (
        db.query(FeedbackLog)
        .filter(FeedbackLog.id.in_(feedback_ids), FeedbackLog.user_id == user_id)
        .all()
    )
