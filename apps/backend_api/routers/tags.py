...
from schemas import TagCreate, TagUpdate, Tag, TagOut
import logging
...
async def get_tags(
    include_usage_count: bool = Query(False, description="Include usage count for each tag"),
    db: Session = Depends(get_db),
    current_user: Dict[str, Any] = Depends(get_current_user)
):
...
async def get_tag_statistics(
    ...
    try:
        ...
        logger.error(f"Error fetching tag statistics for user {current_user['uid']}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_entities_by_tag(
    ...
    try:
        ...
        logger.error(f"Error fetching entities for tag {tag_id} for user {current_user['uid']}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
