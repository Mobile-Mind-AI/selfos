...
async def list_assistant_profiles(
    ...
    profiles = (
        ...
        .filter(AssistantProfile.user_id == user_id)
        .order_by(
...
async def get_assistant_config():
    ...
    return AssistantConfigResponse(
        ...
        default_style=PersonalityStyle(
            formality=50, directness=50, humor=30, empathy=70, motivation=60
        ),
        temperature_ranges={
...
async def complete_onboarding(
    ...
    existing_count = (
        db.query(AssistantProfile).filter(AssistantProfile.user_id == user_id).count()
    )
    ...
    logger.info(
        f"Onboarding completed for user {user_id}: created assistant '{profile.name}'"
    )
...
async def create_assistant_profile(
    ...
    if existing_count >= 5:
        raise HTTPException(
            status_code=400, detail="Maximum of 5 assistant profiles allowed per user"
        )
    ...
    profile = AssistantProfile(
        ...
        or existing_count == 0,  # First profile becomes default
    )
...
async def get_default_assistant_profile(
    ...
    if not profile:
        raise HTTPException(
            status_code=404, detail="No default assistant profile found"
        )
...
async def get_assistant_profile(
    ...
    profile = (
        db.query(AssistantProfile)
        .filter(
            and_(AssistantProfile.id == profile_id, AssistantProfile.user_id == user_id)
        )
        .first()
    )
...
async def delete_assistant_profile(
    ...
    if total_profiles <= 1:
        raise HTTPException(
            status_code=400, detail="Cannot delete the only assistant profile"
        )
...
async def generate_welcome_message(
    profile: AssistantProfile, background_tasks: BackgroundTasks
) -> str:
    ...
    if style.get("empathy", 50) > 70:
        welcome += (
            " I'm here to understand your needs and support you every step of the way."
        )
...
def generate_personality_sample(style: PersonalityStyle, base_message: str) -> str:
    ...
    if style.formality > 70:
        ...
        response = (
            "I would be pleased to assist you in achieving your objectives today."
        )
    ...
    if style.empathy > 80:
        response = response.replace(
            "How can I help", "I'm here for you - how can I help"
        )
