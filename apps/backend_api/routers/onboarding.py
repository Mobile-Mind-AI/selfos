...
def get_onboarding_state(
    ...
    state = (
        ...
        .filter(OnboardingState.user_id == current_user["uid"])
        .first()
    )
    ...
    if not state:
        ...
        print(
            f"🔍 BACKEND: Found onboarding state - completed: {state.onboarding_completed}, steps: {state.completed_steps}"
        )
...
def update_onboarding_step(
    ...
    if request.step == OnboardingStep.ASSISTANT_CREATION:
        ...
        response_data = _handle_assistant_creation(
            request.data, state, current_user, db
        )
    ...
    if step_number not in state.completed_steps:
        ...
        print(
            f"🎯 ONBOARDING: Added step {step_number} to completed_steps: {state.completed_steps}"
        )
...
async def complete_onboarding(
    ...
    if not state:
        ...
        print(
            f"🎯 ONBOARDING: No state found for user {current_user['uid']}, checking for assistant profile..."
        )

        # Check if user has access to any assistant profiles
        user_assistants = await PermissionService.get_user_assistants(
            current_user["uid"], db
        )
    ...
    print(
        f"🎯 ONBOARDING: Current state - completed_steps: {state.completed_steps}, onboarding_completed: {state.onboarding_completed}"
    )
    ...
    if not state.selected_life_areas or len(state.selected_life_areas) == 0:
        user_life_areas = (
            db.query(LifeArea).filter(LifeArea.user_id == current_user["uid"]).all()
        )
        if user_life_areas:
    ...
    print(
        f"🎯 ONBOARDING: Validation - has_assistant: {has_assistant}, has_life_areas: {has_life_areas}"
    )
    ...
    if not has_assistant:
        ...
        if assistant:
            print(
                f"🎯 ONBOARDING: Found assistant profile {assistant.id}, linking to state"
            )
            ...
            for a in user_assistants:
                perm = await PermissionService.get_user_permission_level(
                    current_user["uid"], a.id, db
                )
                assistant_info.append((a.id, a.name, a.is_default, perm))
            print(
                f"🎯 ONBOARDING: User has access to {len(user_assistants)} assistants: {assistant_info}"
            )
    ...
    assistant = (
        ...
        .filter(AssistantProfile.id == state.assistant_profile_id)
        .first()
    )

    welcome_message = (
        f"Welcome! I'm {assistant.name if assistant else 'your assistant'}. "
    )
...
def _handle_assistant_creation(
    ...
    if existing_assistant:
        ...
        existing_assistant.requires_confirmation = data.get(
            "requires_confirmation", True
        )
        existing_assistant.style = data.get(
            "style",
            {
                "formality": 50,
                "directness": 50,
...
def _handle_personality_setup(
    ...
    return {"style": setup_data.style.model_dump()}
...
def _handle_life_areas(
    ...
    if areas_data.custom_life_areas:
        for area_name in areas_data.custom_life_areas:
            life_area = LifeArea(user_id=current_user["uid"], name=area_name, weight=10)
            db.add(life_area)
            db.flush()
...
def _handle_first_goal(
    ...
    return {"goal_id": goal.id, "task_id": state.first_task_id}
...
def _handle_personal_config(
    ...
    from models import CustomLifeArea, PersonalProfile

    try:
        print(f"🔄 ONBOARDING: _handle_personal_config called with data: {data}")
        print(f"🔄 ONBOARDING: User ID: {current_user['uid']}")
        print(
            f"🔄 ONBOARDING: Custom life areas in data: {data.get('custom_life_areas', [])}"
        )
        ...
        profile_data = {
            ...
            "preferences": data.get(
                "preferences", {}
            ),  # Only for extra/custom preferences
            "custom_answers": data.get("custom_answers", {}),
            "selected_life_areas": data.get(
                "selected_life_areas", []
            ),  # Store selected life area IDs
        }
        ...
        if existing_profile:
            ...
            new_profile = PersonalProfile(
                id=str(uuid.uuid4()), user_id=current_user["uid"], **profile_data
            )
            db.add(new_profile)
...
def preview_personality(
    ...
    if motivation > 70:
        encouragement = (
            "You're doing amazing! Every step counts. Let's crush those goals! 💪"
        )
    elif motivation < 30:
