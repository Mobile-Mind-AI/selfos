    ...
    try:
        if req.provider == "email":
            ...
            if req.provider == "google":
                ...
                email = (
                    req.email
                    or f"apple_user_{req.social_token[:8]}@privaterelay.appleid.com"
                )
    ...
    try:
        ...
        try:
            ...
            email_sent = email_service.send_password_reset_email(
                to_email=email, reset_link=link, user_name=user_name
            )
        ...
        if "quota" in error_message or "limit" in error_message:
            return {
                "message": "Password reset service is temporarily unavailable due to high demand. Please try again later."
            }
        elif "configuration" in error_message or "not configured" in error_message:
            return {
                "message": "Password reset service is not properly configured. Please contact support."
            }
        else:
            return {
