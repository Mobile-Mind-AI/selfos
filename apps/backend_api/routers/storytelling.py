...
async def get_story_prompts(
    task_data: dict, current_user: dict = Depends(get_current_user)
):
    """
...
def get_recent_stories(
    ...
    try:
        ...
        stories = (
            ...
            .filter(StorySession.user_id == current_user["uid"])
            .order_by(StorySession.generated_at.desc())
            .limit(limit)
            .all()
        )
            ...
            "stories": [
                {
                    ...
                    "title": story.title,
                    "generated_text": (
                        story.generated_text[:200] + "..."
                        if len(story.generated_text) > 200
                        else story.generated_text
                    ),
                    ...
                    "generated_at": (
                        story.generated_at.isoformat() if story.generated_at else None
                    ),
                    "processing_status": story.processing_status,
                    "source_tasks_count": (
                        len(story.source_tasks) if story.source_tasks else 0
                    ),
                    "source_goals_count": (
                        len(story.source_goals) if story.source_goals else 0
                    ),
                }
...
def get_story_details(
    ...
    try:
        ...
        story = (
            db.query(StorySession)
            .filter(
                StorySession.id == story_id, StorySession.user_id == current_user["uid"]
            )
            .first()
        )
