    ...
    try:
        ...
        result = await conversation_manager.process_message(
            ...
            conversation_context={
                ...
                "assistant_profile": (
                    assistant_profile.id if assistant_profile else None
                ),
            },
...
async def get_user_sessions(
    ...
    try:
        ...
        sessions = (
            query.order_by(desc(ConversationSession.last_activity))
            .offset(offset)
            .limit(limit)
            .all()
        )

        return [ConversationSessionOut.model_validate(session) for session in sessions]
...
async def get_session_logs(
    ...
    try:
        ...
        logs = (
            ...
            .order_by(ConversationLog.conversation_turn)
            .limit(limit)
            .all()
        )

        return [ConversationLogOut.model_validate(log) for log in logs]
...
async def provide_intent_feedback(
    ...
    try:
        ...
        logger.info(
            f"Intent feedback received from user {user_id}: "
            f"{feedback.original_intent} -> {feedback.corrected_intent}"
        )

        return IntentFeedbackOut.model_validate(feedback_entry)
...
async def get_intent_analytics(
    ...
    try:
        ...
        # Average confidence
        avg_confidence = (
            db.query(func.avg(ConversationLog.confidence))
            .filter(
                and_(
                    ConversationLog.user_id == user_id,
                    ConversationLog.created_at >= since_date,
        ...
        fallback_usage_rate = (
            fallback_count / total_messages if total_messages > 0 else 0.0
        )

        # Average processing time
        avg_processing_time = (
            db.query(func.avg(ConversationLog.processing_time_ms))
            .filter(
                and_(
                    ConversationLog.user_id == user_id,
                    ConversationLog.created_at >= since_date,
        ...
        success_rate = (
            high_confidence_count / total_messages if total_messages > 0 else 0.0
        )
...
async def get_conversation_analytics(
    ...
    try:
        ...
        avg_turns = (
            db.query(func.avg(ConversationSession.turn_count))
            .filter(
                and_(
                    ConversationSession.user_id == user_id,
                    ConversationSession.started_at >= since_date,
        ...
        # Intent success rate
        total_successful = (
            db.query(func.sum(ConversationSession.successful_intents))
            .filter(
                and_(
                    ConversationSession.user_id == user_id,
                    ConversationSession.started_at >= since_date,
        ...
        total_attempts = (
            db.query(
                func.sum(
                    ConversationSession.successful_intents
                    + ConversationSession.failed_intents
                )
            )
            .filter(
                and_(
        ...
        common_intents = (
            db.query(
                ConversationLog.intent, func.count(ConversationLog.id).label("count")
            )
        ...
        session_lengths = (
            db.query(
                func.extract(
                    "epoch",
                    ConversationSession.last_activity - ConversationSession.started_at,
                )
                / 60
            )
            .filter(
        ...
        avg_session_length = (
            sum(length[0] for length in session_lengths if length[0])
            / len(session_lengths)
            if session_lengths
            else 0.0
        )
...
async def create_conversation_session(
    ...
    try:
        session = ConversationSession(
            id=session_id, user_id=user_id, session_type="chat", status="active"
        )
        ...
        db.commit()
        logger.info(
            f"Created conversation session {session_id} for user {user_id} with assistant {assistant_id}"
        )
    except Exception as e:
            ...
            if total_intents > 0:
                if session.avg_confidence:
                    session.avg_confidence = (
                        session.avg_confidence * (total_intents - 1)
