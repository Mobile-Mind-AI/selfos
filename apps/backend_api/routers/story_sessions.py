...
def get_story_sessions(
    ...
    | None = Query(None, description="Filter by processing status"),
    ...
    story_sessions = (
        query.order_by(desc(StorySession.created_at)).offset(offset).limit(limit).all()
    )
...
def get_story_session(
    ...
    story_session = (
        ...
        .filter(StorySession.id == session_id, StorySession.user_id == user_id)
        .first()
    )

    if not story_session:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Story session not found"
        )
...
def create_story_session(
    ...
    if session_data.source_goals:
        existing_goals = (
            db.query(Goal.id)
            .filter(Goal.id.in_(session_data.source_goals), Goal.user_id == user_id)
            .all()
        )
    ...
    if session_data.source_tasks:
        existing_tasks = (
            db.query(Task.id)
            .filter(Task.id.in_(session_data.source_tasks), Task.user_id == user_id)
            .all()
        )
    ...
    story_session = StorySession(
        user_id=user_id, **session_data.model_dump(exclude_unset=True)
    )
...
def update_story_session(
    ...
    if "source_goals" in update_data and update_data["source_goals"]:
        existing_goals = (
            db.query(Goal.id)
            .filter(Goal.id.in_(update_data["source_goals"]), Goal.user_id == user_id)
            .all()
        )
    ...
    if "source_tasks" in update_data and update_data["source_tasks"]:
        existing_tasks = (
            db.query(Task.id)
            .filter(Task.id.in_(update_data["source_tasks"]), Task.user_id == user_id)
            .all()
        )
...
def get_story_sessions_summary(
    ...
    base_query = db.query(StorySession).filter(
        StorySession.user_id == user_id, StorySession.created_at >= start_date
    )
    ...
    content_type_counts = (
        db.query(StorySession.content_type, func.count(StorySession.id).label("count"))
        .filter(StorySession.user_id == user_id, StorySession.created_at >= start_date)
        .group_by(StorySession.content_type)
        .all()
    )
    ...
    posting_status_counts = (
        db.query(
            StorySession.posting_status, func.count(StorySession.id).label("count")
        )
        .filter(StorySession.user_id == user_id, StorySession.created_at >= start_date)
        .group_by(StorySession.posting_status)
        .all()
    )
    ...
    processing_status_counts = (
        db.query(
            StorySession.processing_status, func.count(StorySession.id).label("count")
        )
        .filter(StorySession.user_id == user_id, StorySession.created_at >= start_date)
        .group_by(StorySession.processing_status)
        .all()
    )
    ...
    total_word_count_result = (
        db.query(func.sum(StorySession.word_count))
        .filter(
    ...
    avg_rating_result = (
        db.query(func.avg(StorySession.user_rating))
        .filter(
            StorySession.user_id == user_id,
            StorySession.created_at >= start_date,
...
def generate_story(
    ...
    if generation_request.include_goals:
        ...
        if generation_request.include_life_areas:
            goals_query = goals_query.filter(
                Goal.life_area_id.in_(generation_request.include_life_areas)
            )
    ...
    if generation_request.include_tasks:
        ...
        if generation_request.include_life_areas:
            tasks_query = tasks_query.filter(
                Task.life_area_id.in_(generation_request.include_life_areas)
            )
...
def process_story_generation(session_id: str, db: Session):
    ...
    if story_session:
        ...
        story_session.estimated_read_time = (
            story_session.word_count * 4
        )  # Assuming 250 WPM reading speed
...
def publish_story_session(
    ...
    if (
        publish_request.scheduled_time
        and publish_request.scheduled_time > datetime.utcnow()
    ):
        ...
        "content_types": [
            {
                "value": "summary",
                "label": "Summary",
                "description": "Factual summary of activities and achievements",
            },
            {
                "value": "story",
                "label": "Story",
                "description": "Narrative story format with creative elements",
            },
            {
                "value": "reflection",
                "label": "Reflection",
                "description": "Thoughtful reflection on progress and insights",
            },
            {
                "value": "achievement",
                "label": "Achievement",
                "description": "Celebration of accomplishments and milestones",
            },
        ]
    }
    ...
    return {
        "periods": [
            ...
            },
            {
                "value": "project-based",
                "label": "Project-based",
                "description": "Based on specific project completion",
            },
        ...
        "platforms": [
            {
                "value": "instagram",
                "label": "Instagram",
                "description": "Instagram posts and stories",
            },
            {
                "value": "youtube",
                "label": "YouTube",
            ...
            },
            {
                "value": "facebook",
                "label": "Facebook",
                "description": "Facebook posts and stories",
            },
            {"value": "tiktok", "label": "TikTok", "description": "TikTok videos"},
            {
                "value": "blog",
                "label": "Personal Blog",
                "description": "Personal blog or website",
            },
        ]
    }
