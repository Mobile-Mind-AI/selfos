...
@router.post(
    "/media-attachments", response_model=schemas.MediaAttachment, status_code=201
)
...
@router.get(
    "/media-attachments/{attachment_id}", response_model=schemas.MediaAttachment
)
def get_media_attachment(
    ...
    attachment = (
        db.query(models.MediaAttachment)
        .filter(
    ...
    if not attachment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Media attachment not found"
        )
...
def get_media_statistics(
    ...
    total_attachments = (
        db.query(models.MediaAttachment)
        .filter(models.MediaAttachment.user_id == current_user["uid"])
        .count()
    )
    ...
    file_type_counts = (
        ...
        .filter(models.MediaAttachment.user_id == current_user["uid"])
        .group_by(models.MediaAttachment.media_type)
        .all()
    )

    # Total storage used
    total_size = (
        db.query(func.sum(models.MediaAttachment.file_size))
        .filter(models.MediaAttachment.user_id == current_user["uid"])
        .scalar()
        or 0
    )
