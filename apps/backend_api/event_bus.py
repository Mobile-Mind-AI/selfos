...
def get_subscribers(event_type: EventType = None) -> dict[str, int]:
    ...
    return {
        event_type.value: len(handlers) for event_type, handlers in subscribers.items()
    }
...
def get_event_stats() -> dict:
    ...
    return {
        **event_stats,
        ...
        "health": (
            "healthy"
            if event_stats["errors"] < event_stats["total_published"] * 0.1
            else "degraded"
        ),
    }
...
async def publish_task_completed(task_id: str, user_id: str, task_data: dict = None):
    ...
    """
    payload = {"task_id": task_id, "user_id": user_id, **(task_data or {})}
...
async def publish_goal_completed(goal_id: str, user_id: str, goal_data: dict = None):
    ...
    """
    payload = {"goal_id": goal_id, "user_id": user_id, **(goal_data or {})}
