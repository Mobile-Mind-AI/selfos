...
async def enqueue_segment_generation(
    ...
    """
    try:
        from models import MediaAttachment, StorySession, Task
        ...
        task = (
            db.query(Task)
            .filter(Task.id == int(task_id), Task.user_id == user_id)
            .first()
        )
        ...
        media_attachments = (
            db.query(MediaAttachment).filter(MediaAttachment.task_id == task.id).all()
        )
...
async def _generate_task_story(task, media_attachments: list) -> dict[str, str]:
    ...
    if task.duration:
        ...
        if hours > 0:
            duration_text = (
                f" This took {hours} hours and {minutes} minutes to complete."
            )
        else:
...
async def generate_weekly_summary(db: Session, user_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        for task in completed_tasks:
            area_key = (
                f"area_{task.life_area_id}"
                if task.life_area_id
                else f"goal_{task.goal_id}"
            )
        ...
        for area_key, tasks in areas.items():
            ...
            if len(tasks) == 1:
                story_parts.append(
                    f"In one area, '{task_titles[0]}' was completed successfully."
                )
            else:
                story_parts.append(
                    f"Significant progress was made with tasks including '{task_titles[0]}' and {len(tasks)-1} other achievements."
                )

        story_parts.append(
            "Each completion builds toward larger objectives and represents meaningful progress in the journey of personal development."
        )
...
async def suggest_story_prompts(task_data: dict[str, Any]) -> list[str]:
    ...
    prompts.append(
        f"Write an inspiring story about completing '{title}'. Focus on the sense of accomplishment and progress."
    )

    # Context-specific prompts
    if description:
        prompts.append(
            f"Create a narrative about the journey of completing '{title}'. Include details about: {description}"
        )

    if has_media:
        prompts.append(
            f"Tell the story of '{title}' completion, incorporating visual elements and media that documented the process."
        )

    if life_area:
        prompts.append(
            f"Write about how completing '{title}' contributes to overall growth in this life area."
        )

    # Motivational variants
    prompts.append(
        f"Describe the personal growth achieved through completing '{title}', emphasizing lessons learned and future potential."
    )
