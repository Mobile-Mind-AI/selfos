...
ai_engine_path = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "ai_engine"
)
...
class IntentClassifier:
    ...
    def __init__(self):
        try:
            ...
            logger.warning(
                f"Failed to initialize AI orchestrator: {e}, falling back to rule-based classification"
            )
    ...
    async def classify_intent(
        ...
        try:
            ...
            if self.ai_orchestrator and AI_MODELS_AVAILABLE:
                result = await self._llm_classify(
                    message, user_context, assistant_profile
                )

                # If confidence is low, try rule-based fallback
                if result.confidence < self.confidence_threshold:
                    logger.info(
                        f"LLM confidence {result.confidence:.2f} below threshold, trying rule-based fallback"
                    )
                ...
                logger.info(
                    "AI orchestrator not available, using rule-based classification"
                )
        ...
        result.entities.update(
            self._extract_entities(message, result.intent, result.entities)
        )
    ...
    async def _llm_classify(
        ...
        if not AI_MODELS_AVAILABLE:
            logger.warning(
                "AI models not available, falling back to rule-based classification"
            )
        ...
        intent_temperature = (
            0.1  # Default low temperature for consistent classification
        )
        ...
        for intent_type, patterns in self.intent_patterns.items():
            ...
            for pattern in patterns:
                if re.search(pattern, message_lower, re.IGNORECASE):
                    matches += 1
                    confidence = min(
                        0.95, 0.7 + (matches * 0.1)
                    )  # Max 0.95 for rule-based
    ...
    def _extract_entities(
        ...
        if (
            intent in ["create_goal", "create_task", "create_project"]
            and "title" not in existing_entities
        ):
...
Be conservative with confidence scores. Use confidence < 0.85 for ambiguous messages."""
    ...
    async def _log_conversation(
        ...
        logger.info(
            f"Intent classified: {result.intent} (confidence: {result.confidence:.2f}, "
...
class ConversationFlowManager:
    ...
    async def process_message(
        ...
        result = await self.intent_classifier.classify_intent(
            message, user_context, assistant_profile=assistant_profile
        )
    ...
    def _update_conversation_state(self, user_id: str, result: IntentResult) -> Dict:
        ...
        missing_entities = [
            entity for entity in required_entities if entity not in result.entities
        ]
    ...
    def _determine_next_actions(
        ...
        if result.confidence < 0.85:
            ...
            actions.append({"type": "provide_advice", "context": result.entities})

        elif result.intent == "chat_continuation":
            actions.append(
                {"type": "continue_conversation", "context": conversation_state}
            )
