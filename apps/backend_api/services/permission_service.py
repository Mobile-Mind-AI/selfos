...
class PermissionService:
    ...
    async def check_permission(
        ...
        permission = db.query(AssistantPermission).filter(
    ...
    async def get_user_assistants(
        ...
        # Direct ownership
        owned = db.query(AssistantProfile).filter(
            AssistantProfile.owner_id == user_id
        ).all()
        
        # Shared assistants (with valid permissions)
        shared_assistant_ids = db.query(AssistantPermission.assistant_id).filter(
            AssistantPermission.user_id == user_id,
            (AssistantPermission.expires_at.is_(None)) | 
            (AssistantPermission.expires_at > datetime.utcnow())
        ).subquery()
        
        shared = db.query(AssistantProfile).filter(
            AssistantProfile.id.in_(shared_assistant_ids)
        ).all()
        
        # Public assistants (read-only) - exclude owned to avoid duplicates
        public = db.query(AssistantProfile).filter(
    ...
    async def share_assistant(
        ...
        if not can_share:
            raise HTTPException(
                status_code=403, 
                detail="Insufficient permissions to share assistant"
            )
        ...
        if cls._HIERARCHY.get(permission_level, 0) > cls._HIERARCHY.get(granter_permission, 0):
            raise HTTPException(
                status_code=403,
                detail="Cannot grant higher permission than you have"
            )
            
        # Create or update permission
        existing_permission = db.query(AssistantPermission).filter(
            AssistantPermission.assistant_id == assistant_id,
    ...
    async def cleanup_expired_permissions(cls, db: Session) -> int:
        ...
        expired_permissions = db.query(AssistantPermission).filter(
