    ...
    try:
        ...
        if goals:
            ...
            if area_completions:
                ...
                area = (
                    db.query(LifeArea)
                    .filter(LifeArea.id == most_productive_area_id)
                    .first()
                )
        ...
        if goal_completion_rate < 50:
            recommendations.append(
                "Focus on completing existing goals before starting new ones"
            )
...
async def predict_completion_date(
    ...
    try:
        ...
        velocity_per_day = (
            recent_completions / 14 if recent_completions > 0 else 0.1
        )  # Fallback
