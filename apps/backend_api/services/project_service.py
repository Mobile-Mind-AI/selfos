...
class ProjectService:
    ...
    def create_project(
        self, db: Session, user_id: str, project_data: schemas.ProjectCreate
    ) -> models.Project:
        """
        ...
        try:
            ...
            if project_data.parent_id:
                ...
                if not parent:
                    raise ValueError(
                        f"Parent project {project_data.parent_id} not found"
                    )
            ...
            parent_info = (
                f" (parent: {project_data.parent_id})" if project_data.parent_id else ""
            )
            logger.info(
                f"Created project {db_project.id} '{db_project.title}' for user {user_id}{parent_info}"
            )
            return db_project
        except SQLAlchemyError as e:
    ...
    def update_project(
        ...
        try:
            ...
            if not project:
                logger.warning(
                    f"Project {project_id} not found for update by user {user_id}"
                )
                return None
            ...
            if project_data.parent_id and project_data.parent_id != project.parent_id:
                ...
                if self._would_create_cycle(
                    db, user_id, project_id, project_data.parent_id
                ):
            ...
            logger.info(
                f"Updated project {project_id} for user {user_id}. Progress: {old_progress}% -> {project.progress}%{parent_change}"
            )
            return project
        except SQLAlchemyError as e:
    ...
    def delete_project(self, db: Session, user_id: str, project_id: int) -> bool:
        ...
        try:
            ...
            if not project:
                logger.warning(
                    f"Project {project_id} not found for deletion by user {user_id}"
                )
                return False
            ...
            logger.info(
                f"Deleted project {project_id} '{project.title}' for user {user_id}"
            )
            return True
        except SQLAlchemyError as e:
    ...
    def get_projects_by_life_area(
        ...
        try:
            ...
            try:
                logger.info(
                    f"Retrieved {len(projects)} projects for life area {life_area_id}, user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(
                    f"Retrieved projects for life area {life_area_id}, user {user_id}"
                )
            return projects
        except SQLAlchemyError as e:
            logger.error(
                f"Database error retrieving projects for life area {life_area_id}: {e}"
            )
            raise
    ...
    def get_projects_by_status(
        ...
        try:
            projects = (
                ...
                .filter(
                    models.Project.user_id == user_id, models.Project.status == status
                )
                .all()
            )
            ...
            try:
                logger.info(
                    f"Retrieved {len(projects)} projects with status '{status}' for user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(
                    f"Retrieved projects with status '{status}' for user {user_id}"
                )
            return projects
        except SQLAlchemyError as e:
            logger.error(
                f"Database error retrieving projects with status '{status}': {e}"
            )
            raise
    ...
    def _would_create_cycle(
        self, db: Session, user_id: str, project_id: int, new_parent_id: int
    ) -> bool:
    ...
    def get_children(
        ...
        try:
            children = (
                db.query(models.Project)
                .options(joinedload(models.Project.life_area))
                .filter(
            ...
            try:
                logger.info(
                    f"Retrieved {len(children)} children for project {parent_id}, user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(
                    f"Retrieved children for project {parent_id}, user {user_id}"
                )
            return children
        except SQLAlchemyError as e:
            logger.error(
                f"Database error getting children for project {parent_id}: {e}"
            )
            raise
    ...
    def get_descendants(
        ...
        try:
            ...
            try:
                logger.info(
                    f"Retrieved {len(descendants)} descendants for project {parent_id}, user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(
                    f"Retrieved descendants for project {parent_id}, user {user_id}"
                )
            return descendants
        except Exception as e:
        ...
        while current_project:
            ...
            if current_project.parent_id:
                current_project = self.get_project(
                    db, user_id, current_project.parent_id
                )
            else:
    ...
    def get_root_projects(self, db: Session, user_id: str) -> list[models.Project]:
        ...
        try:
            ...
            try:
                logger.info(
                    f"Retrieved {len(projects)} root projects for user {user_id}"
                )
            except (TypeError, AttributeError):
            ...
            logger.error(
                f"Database error retrieving root projects for user {user_id}: {e}"
            )
            raise
    ...
    def move_project(
        ...
        try:
            ...
            logger.info(
                f"Moved project {project_id} from parent {old_parent} to {new_parent_id} for user {user_id}"
            )
            return project
    ...
    def get_ancestors(
        ...
        try:
            ...
            try:
                logger.info(
                    f"Retrieved {len(ancestors)} ancestors for project {project_id}, user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(
                    f"Retrieved ancestors for project {project_id}, user {user_id}"
                )
    ...
    def get_project_tree(
        ...
        try:
            ...
            if root_id:
                ...
                root_projects = self.get_root_projects_filtered(
                    db, user_id, life_area_id
                )
    ...
    def get_root_projects_filtered(
        ...
        try:
            ...
            try:
                logger.info(
                    f"Retrieved {len(root_projects)} root projects for user {user_id}"
                )
            except (TypeError, AttributeError):
                logger.info(f"Retrieved root projects for user {user_id}")
            ...
            logger.error(
                f"Database error getting root projects for user {user_id}: {e}"
            )
            raise
    ...
    def _calculate_depth(
        self, db: Session, user_id: str, project_id: int, current_depth: int = 0
    ) -> int:
    ...
    def get_project_tree_structure(
        ...
        try:
            ...
            logger.info(
                f"Built project tree with {len(tree)} root nodes for user {user_id}"
            )
            return tree
    ...
    def _build_project_tree_node(
        ...
        try:
            ...
            for child in children:
                ...
                child_node = self._build_project_tree_node(
                    db, user_id, child, branch_visited, depth + 1, max_depth
                )
