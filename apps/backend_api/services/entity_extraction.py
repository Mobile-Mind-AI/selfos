...
class EntityExtractor:
    ...
    def _load_entity_types(self):
        ...
        self.entity_types = {}
        types = self.db.query(EntityType).filter(
            or_(
                EntityType.user_id == self.user_id,
    ...
    def _extract_places(self, text: str) -> List[Dict[str, Any]]:
        ...
        place_indicators = [
            'at', 'in', 'near', 'from', 'to', 'visit', 'located',
            'office', 'building', 'street', 'avenue', 'road', 'city',
            'country', 'state', 'park', 'restaurant', 'hotel', 'airport'
        ]
    ...
    def _extract_organizations(self, text: str) -> List[Dict[str, Any]]:
        ...
        org_suffixes = ['Inc', 'LLC', 'Corp', 'Corporation', 'Company', 
                       'Ltd', 'Limited', 'Group', 'Institute', 'University',
                       'College', 'School', 'Hospital', 'Bank', 'Agency']
    ...
    def _is_common_phrase(self, text: str) -> bool:
        ...
        common_phrases = {
            'The', 'This', 'That', 'These', 'Those', 'Some', 'Many', 'Few',
            'All', 'Any', 'Each', 'Every', 'No', 'None', 'Several', 'Various',
            'New York', 'Los Angeles', 'San Francisco'  # Add common place names
        }
    ...
    def _create_association(
        ...
        if context_type == 'goal':
            existing = self.db.query(GoalEntity).filter(
                GoalEntity.goal_id == context_id,
                GoalEntity.entity_id == entity_id
            ).first()
            
            if not existing:
                association = GoalEntity(
                    goal_id=context_id,
                    entity_id=entity_id
                )
            ...
            existing = self.db.query(ProjectEntity).filter(
            ...
            existing = self.db.query(TaskEntity).filter(
                TaskEntity.task_id == context_id,
                TaskEntity.entity_id == entity_id
            ).first()
            
            if not existing:
                association = TaskEntity(
                    task_id=context_id,
                    entity_id=entity_id
                )
                self.db.add(association)
    ...
    def analyze_entity_importance(self, entity_id: int) -> float:
        ...
        # Count associations
        goal_count = self.db.query(GoalEntity).filter(
            GoalEntity.entity_id == entity_id
        ).count()
        score += min(goal_count * 0.1, factors['goal_associations'])
        
        project_count = self.db.query(ProjectEntity).filter(
            ProjectEntity.entity_id == entity_id
        ).count()
        score += min(project_count * 0.05, factors['project_associations'])
        
        task_count = self.db.query(TaskEntity).filter(
            TaskEntity.entity_id == entity_id
        ).count()
        score += min(task_count * 0.03, factors['task_associations'])
        
        # Count relationships
        relationship_count = self.db.query(EntityRelationship).filter(
            or_(
        ...
        if entity.last_interaction:
            days_since_interaction = (
                datetime.utcnow() - entity.last_interaction
            ).days
...
class EntityExtractionService:
    ...
    def extract_from_project(self, project: Project, db: Session) -> List[Dict[str, Any]]:
...
def extract_entities_from_project(project: Project, db: Session) -> List[Dict[str, Any]]:
