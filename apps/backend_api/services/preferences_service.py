...
def update_user_preferences_with_history(
    ...
    current_prefs = db.query(UserPreferences).filter(
...
def get_preference_change_summary(
    ...
    history_records = db.query(UserPreferencesHistory).filter(
        UserPreferencesHistory.user_id == user_id,
    ...
    most_changed = max(change_counts.items(), key=lambda x: x[1]) if change_counts else None
