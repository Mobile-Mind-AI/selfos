...
class LocalEmbeddingProvider(EmbeddingProvider):
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        embedding = await loop.run_in_executor(
            None, lambda: model.encode([text])[0].tolist()
        )
        return embedding
    ...
    async def search(
        ...
        async with self._lock:
            ...
            for entry in self.entries.values():
                ...
                result = SearchResult(
                    entry=entry, similarity_score=similarity, context_relevance=1.0
                )
                results.append(result)
        ...
        async with self._lock:
            ...
            for entry_id in entry_ids:
                if (
                    ...
                    and self.entries[entry_id].user_id == user_id
                ):
    ...
    async def get_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "newest_entry": (
                    max(e.created_at for e in user_entries) if user_entries else None
                ),
            }
...
class EnhancedMemoryService:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embedding_provider = (
            embedding_provider or self._create_default_embedding_provider()
        )
        ...
        if self.vector_store_type == VectorStoreType.PINECONE:
            return PineconeVectorStore(
                ...
                index_name=kwargs.get(
                    "pinecone_index", os.getenv("PINECONE_INDEX", "selfos")
                ),
            )
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            "similarity_threshold": float(
                os.getenv("MEMORY_SIMILARITY_THRESHOLD", "0.7")
            ),
    ...
    async def search_memories(
        ...
        try:
            ...
            enhanced_results = await self._enhance_context_relevance(
    ...
    async def _enhance_context_relevance(
        ...
        for result in results:
            ...
            if (
                "task" in query.lower()
                and result.entry.content_type == "task_completion"
            ):
