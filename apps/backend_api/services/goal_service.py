...
class GoalService:
    ...
    def create_goal(
        self, db: Session, user_id: str, goal_data: schemas.GoalCreate
    ) -> models.Goal:
        """
        ...
        try:
            ...
            parent_info = (
                f" (parent: {goal_data.parent_id})" if goal_data.parent_id else ""
            )
            logger.info(
                f"Created goal {db_goal.id} '{db_goal.title}' for user {user_id}{parent_info}"
            )
            return db_goal
        except SQLAlchemyError as e:
    ...
    def update_goal(
        ...
        try:
            ...
            logger.info(
                f"Updated goal {goal_id} for user {user_id}. Progress: {old_progress}% -> {goal.progress}%{parent_change}"
            )
            return goal
        except SQLAlchemyError as e:
    ...
    def delete_goal(self, db: Session, user_id: str, goal_id: int) -> bool:
        ...
        try:
            ...
            if not goal:
                logger.warning(
                    f"Goal {goal_id} not found for deletion by user {user_id}"
                )
                return False
    ...
    def get_goals_by_life_area(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(goals)} goals for life area {life_area_id}, user {user_id}"
            )
            return goals
        except SQLAlchemyError as e:
            logger.error(
                f"Database error retrieving goals for life area {life_area_id}: {e}"
            )
            raise
    ...
    def get_goals_by_status(
        ...
        try:
            goals = (
                ...
                .filter(models.Goal.user_id == user_id, models.Goal.status == status)
                .all()
            )

            logger.info(
                f"Retrieved {len(goals)} goals with status '{status}' for user {user_id}"
            )
            return goals
        except SQLAlchemyError as e:
    ...
    def _would_create_cycle(
        self, db: Session, user_id: str, goal_id: int, new_parent_id: int
    ) -> bool:
        """
    ...
    def get_goal_children(
        ...
        try:
            children = (
                ...
                .filter(
                    models.Goal.user_id == user_id, models.Goal.parent_id == parent_id
                )
                .all()
            )

            logger.info(
                f"Retrieved {len(children)} children for goal {parent_id}, user {user_id}"
            )
            return children
        except SQLAlchemyError as e:
            logger.error(
                f"Database error retrieving goal children for {parent_id}: {e}"
            )
            raise
    ...
    def get_root_goals(self, db: Session, user_id: str) -> list[models.Goal]:
        ...
        try:
            goals = (
                ...
                .filter(models.Goal.user_id == user_id, models.Goal.parent_id.is_(None))
                .all()
            )
            ...
            logger.error(
                f"Database error retrieving root goals for user {user_id}: {e}"
            )
            raise
    ...
    def move_goal(
        ...
        try:
            ...
            logger.info(
                f"Moved goal {goal_id} from parent {old_parent} to {new_parent_id} for user {user_id}"
            )
            return goal
    ...
    def get_goal_tree(
        ...
        try:
            ...
            logger.info(
                f"Built goal tree with {len(tree)} root nodes for user {user_id}"
            )
            return tree
    ...
    def _build_goal_tree_node(
        self, db: Session, user_id: str, goal: models.Goal
