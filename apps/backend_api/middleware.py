...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_http_exception(
        self, request_id: str, error: HTTPException
    ) -> JSONResponse:
    ...
    async def _handle_integrity_error(
        self, request_id: str, error: IntegrityError
    ) -> JSONResponse:
    ...
    async def _handle_database_error(
        self, request_id: str, error: SQLAlchemyError
    ) -> JSONResponse:
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (
            request.url.path in ["/health", "/health/basic"]
    ...
    def _check_rate_limits(
        ...
        if burst_count >= self.burst_limit:
            logger.warning(
                f"Burst rate limit exceeded for {client_ip}: {burst_count} requests in 10 seconds"
            )
            return JSONResponse(
                status_code=429,
                content={
        ...
        if minute_count >= self.requests_per_minute:
            logger.warning(
                f"Minute rate limit exceeded for {client_ip}: {minute_count} requests per minute"
            )
            return JSONResponse(
                status_code=429,
                content={
        ...
        if hour_count >= self.requests_per_hour:
            logger.warning(
                f"Hour rate limit exceeded for {client_ip}: {hour_count} requests per hour"
            )
            return JSONResponse(
                status_code=429,
                content={
    ...
    def _add_rate_limit_headers(
        ...
        minute_count = sum(
            1
            for timestamp in self.minute_windows[client_ip]
            if timestamp > current_time - 60
        )
        hour_count = sum(
            1
            for timestamp in self.hour_windows[client_ip]
