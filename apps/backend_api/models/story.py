...
class StorySession(Base):
    ...
    )  # "productivity", "wellness", "achievement", etc.
    ...
    style = Column(
        String, nullable=True, default="documentary"
    )  # "documentary", "motivational", "recap"
    voice_type = Column(String, nullable=True, default="natural")  # TTS voice selection

    # Status tracking
    status = Column(
        Enum(
            "draft", "generating", "ready", "published", "archived", name="story_status"
        ),
        nullable=False,
        default="draft",
    )
    ...
    )  # "true"/"false" as string for consistency
    share_token = Column(
        String, nullable=True, unique=True
    )  # For sharing private stories
    view_count = Column(Integer, nullable=False, default=0)

    # Export details
    exported_platforms = Column(
...
Index(
    "ix_story_public_published",
    StorySession.is_public,
    StorySession.published_at.desc(),
)
