...
from .associations import habit_tags
...
class Habit(Base):
    ...
    habit_type = Column(
        String, nullable=False, default="build"
    )  # build, break, maintain

    # Habit configuration
    recurrence_rule = Column(
        JSON, nullable=False
    )  # Stores the recurrence configuration
    frequency = Column(String, nullable=False, default="daily")  # daily, weekly, custom
    frequency_details = Column(
        JSON, nullable=True
    )  # {"days": ["mon", "wed", "fri"]} or {"times_per_week": 3}
    ...
    best_streak = Column(Integer, nullable=False, default=0)  # Alias for longest_streak
    ...
    color = Column(String, nullable=True)  # Color preference for UI
    ...
    success_rate = Column(
        JSON, nullable=True
    )  # {"last_7_days": 0.85, "last_30_days": 0.72}
    difficulty_rating = Column(
        Integer, nullable=True
    )  # User's rating of difficulty (1-5)
    ...
    end_date = Column(DateTime, nullable=True)  # Optional end date for temporary habits
...
class HabitCompletion(Base):
    ...
    intensity_rating = Column(Integer, nullable=True)  # Intensity rating 1-10
...
Index(
    "ix_completion_habit_date",
    HabitCompletion.habit_id,
    HabitCompletion.completion_date.desc(),
)
Index(
    "ix_completion_user_date",
    HabitCompletion.user_id,
    HabitCompletion.completion_date.desc(),
)
