...
from services import notifications, progress, storytelling
...
async def handle_task_completed(payload: Dict[str, Any]):
    ...
    try:
        ...
        services_tasks.append(
            _safe_service_call(
                "enhanced_memory.store_memory", _store_task_memory(task_data)
            )
        )
        ...
        for i, result in enumerate(results):
            service_name = (
                service_names[i] if i < len(service_names) else f"service_{i}"
            )
        ...
        logger.info(
            f"Task completion processing finished: {success_count}/{len(results)} services succeeded"
        )
...
async def handle_goal_completed(payload: Dict[str, Any]):
    ...
    try:
        ...
        result = await _safe_service_call(
            ...
            notifications.send_goal_completion_notification(
                user_id, goal_title, completion_stats
            ),
        )
    ...
    try:
        ...
        if task_data.get("media_count", 0) > 0:
            content_parts.append(
                f"Documented with {task_data['media_count']} media attachments"
            )
        ...
        return {"success": True, "entry_id": entry_id, "content_length": len(content)}
        ...
        goal_payload = {
            ...
            "goal_data": {
                ...
                "completion_stats": {
                    ...
                    "duration_days": (
                        (goal.updated_at - goal.created_at).days
                        if goal.updated_at and goal.created_at
                        else None
                    ),
                },
            },
        ...
        for user in users:
            try:
                ...
                if (
                    ...
                    and summary_result.get("task_count", 0) > 0
                ):
                    # Send weekly summary notification
                    notification_result = (
                        await notifications.send_weekly_summary_notification(
                            user.id, {"tasks_completed": summary_result["task_count"]}
                        )
                    )
                    ...
                    summary_results.append(
                        {
                            ...
                            "notification_sent": notification_result.get(
                                "success", False
                            ),
                        }
                    )
                ...
                logger.error(
                    f"Failed to generate weekly summary for user {user.id}: {e}"
                )

        db.close()

        logger.info(
            f"Weekly summary generation completed: {len(summary_results)} summaries generated"
        )
