...
def archive_story_sessions(
    ...
    archive_query = db.query(StorySession).filter(StorySession.created_at < cutoff_date)
    ...
    try:
        while True:
            ...
            db.query(StorySession).filter(StorySession.id.in_(record_ids)).delete(
            ...
            logger.info(
                f"Archived {len(records)} story sessions (total: {archived_count})"
            )
...
def archive_feedback_logs(
    ...
    # Count records to archive
    archive_query = db.query(FeedbackLog).filter(FeedbackLog.created_at < cutoff_date)
    total_records = archive_query.count()
    ...
    try:
        while True:
            ...
            # Delete from main table
            record_ids = [record.id for record in records]
            db.query(FeedbackLog).filter(FeedbackLog.id.in_(record_ids)).delete(
                synchronize_session=False
            )
            ...
            logger.info(
                f"Archived {len(records)} feedback logs (total: {archived_count})"
            )
...
def get_archival_stats(db: Session) -> dict[str, Any]:
    ...
    """
    from models import FeedbackLog, StorySession
    ...
    try:
        ...
        story_old_count = (
            ...
            .filter(
                StorySession.created_at
                < datetime.utcnow()
                - timedelta(days=ArchivalConfig.STORY_SESSION_RETENTION_DAYS)
            )
            .count()
        )
        ...
        feedback_old_count = (
            ...
            .filter(
                ...
                < datetime.utcnow()
                - timedelta(days=ArchivalConfig.FEEDBACK_LOG_RETENTION_DAYS)
            )
            .count()
        )
        ...
        try:
            story_archive_result = db.execute(
        ...
        try:
            feedback_archive_result = db.execute(
                text("SELECT COUNT(*) FROM feedback_logs_archive")
            ).scalar()
...
def archive_all_tables(db: Session, dry_run: bool = False) -> dict[str, Any]:
    ...
    results["operations"]["feedback_logs"] = archive_feedback_logs(db, dry_run=dry_run)
    ...
    results["summary"] = {
        ...
        "failed_operations": sum(
            1 for op in results["operations"].values() if not op.get("success", True)
        ),
    }
