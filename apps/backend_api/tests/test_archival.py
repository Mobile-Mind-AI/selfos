...
from db import archive_all_tables, create_archive_tables, engine, get_archival_stats
from dependencies import get_db
from models import Base, FeedbackLog, StorySession
...
def test_archival():
    ...
    try:
        ...
        print(
            f"Story Sessions - Archivable: {stats['story_sessions']['archivable_records']}"
        )
        print(f"Feedback Logs - Active: {stats['feedback_logs']['active_records']}")
        print(
            f"Feedback Logs - Archivable: {stats['feedback_logs']['archivable_records']}"
        )
        ...
        print(
            f"Story Sessions - Active after archival: {final_stats['story_sessions']['active_records']}"
        )
        print(
            f"Feedback Logs - Active after archival: {final_stats['feedback_logs']['active_records']}"
        )
        ...
        try:
            ...
            story_archive_count = db.execute(
                text("SELECT COUNT(*) FROM story_sessions_archive")
            ).scalar()
            feedback_archive_count = db.execute(
