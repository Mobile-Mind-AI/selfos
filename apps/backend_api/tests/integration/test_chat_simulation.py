...
class TestChatSimulation:
    ...
    def test_new_user_onboarding_chat(self, get_test_user_headers):
        ...
        assert any(
            word in content_lower for word in ["health", "skill", "goal", "specific"]
        )
    ...
    def test_goal_clarification_conversation(self, get_test_user_headers):
        ...
        for turn in conversation_flow:
            ...
            has_expected = any(
                keyword in content_lower for keyword in turn["expected_keywords"]
            )
            assert (
                has_expected
            ), f"Expected keywords {turn['expected_keywords']} not found in: {ai_response['content']}"
    ...
    def test_goal_to_tasks_conversation_flow(self, get_test_user_headers):
        ...
        assert ai_response["intent_detected"] in [
            "goal_setting",
            None,
        ]  # None is acceptable for mock
        ...
        goal_request = {
            ...
            "life_areas": [
                {"id": 1, "name": "Health", "description": "Physical fitness"}
            ],
            "existing_goals": [],
            "user_preferences": {
                "fitness_level": "beginner",
                "time_available": "30min daily",
            },
        ...
        chat_request2 = {
            ...
            "conversation_history": conversation_history,
            "user_context": {"recent_goal": "5K running", "plan_provided": True},
        }
        ...
        assert any(
            ...
            for word in ["track", "progress", "measure", "log", "record"]
        )
    ...
    def test_motivation_and_obstacles_conversation(self, get_test_user_headers):
        ...
        for turn in motivation_flow:
            ...
            if turn["expected_response_type"] == "support":
                assert any(
                    word in content_lower
                    for word in ["understand", "common", "normal", "help"]
                )
            elif turn["expected_response_type"] == "solution":
                assert any(
                    word in content_lower
                    for word in ["try", "consider", "schedule", "time"]
                )
            elif turn["expected_response_type"] == "encouragement":
                assert any(
                    word in content_lower
                    for word in ["great", "good", "excellent", "smart"]
                )
    ...
    def test_progress_review_conversation(self, get_test_user_headers):
        ...
        for i, message in enumerate(progress_conversation):
            ...
            if i == 0:  # First message - celebrating progress
                assert any(
                    word in content_lower
                    for word in ["great", "good", "progress", "well done"]
                )
            elif i == 1:  # Second message - addressing time constraints
                assert any(
                    word in content_lower
                    for word in ["time", "schedule", "busy", "understand"]
                )
            elif i == 2:  # Third message - providing guidance
                assert any(
                    word in content_lower
                    for word in ["adjust", "realistic", "suggest", "consider"]
                )
    ...
    def test_multi_goal_management_conversation(self, get_test_user_headers):
        ...
        for message in multi_goal_flow:
            ...
            if "overwhelmed" in message:
                assert any(
                    word in content_lower
                    for word in ["prioritize", "focus", "one", "balance"]
                )
            if "balance" in message:
                assert any(
                    word in content_lower
                    for word in ["schedule", "time", "realistic", "small"]
                )
...
class TestChatMemoryIntegration:
    ...
    def test_chat_with_memory_context(self, get_test_user_headers):
        ...
        content_lower = ai_response2["content"].lower()
        assert any(
            word in content_lower for word in ["exercise", "fitness", "start", "plan"]
        )
...
class TestChatWorkflows:
    ...
    def test_complete_goal_planning_workflow(self, get_test_user_headers):
        ...
        for step in workflow_steps:
            ...
            assert (
                has_expected
            ), f"Step {step['step']}: Expected {step['expected']} in response"
...
class TestChatPerformance:
    ...
    def test_long_conversation_performance(self, get_test_user_headers):
        ...
        for i in range(10):  # 10 turns = 20 messages in history
            user_message = (
                f"This is message {i+1} in our conversation about my fitness goals"
            )
...
@pytest.mark.skip(
    reason="Authentication tests don't work properly with dependency overrides in conftest"
)
