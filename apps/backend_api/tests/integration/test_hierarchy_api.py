...
from models import Goal, Project, LifeArea
...
class TestHierarchyAPI:
    ...
    def create_test_project(self, db: Session, title: str, parent_id: int = None) -> Project:
    ...
    def test_move_goal_api_success(self, client: TestClient, db: Session):
        ...
        response = client.put(
            f"/api/goals/{child_goal.id}/move",
            json={"parent_id": parent_goal.id}
        )
    ...
    def test_move_goal_api_cycle_error(self, client: TestClient, db: Session):
        ...
        response = client.put(
            f"/api/goals/{parent_goal.id}/move",
            json={"parent_id": child_goal.id}
        )
        
        # Verify error response
        assert response.status_code == 400
        assert "Moving goal would create a cycle in hierarchy" in response.json()["detail"]
    ...
    def test_hierarchy_api_input_validation(self, client: TestClient, db: Session):
        ...
        response = client.put(f"/api/goals/{goal.id}/move", json={"parent_id": "invalid"})
        assert response.status_code == 422
    ...
    def test_complete_hierarchy_workflow(self, client: TestClient, db: Session):
        ...
        career_branch = next((g for g in tree if g["title"] == "Career Development"), None)
        ...
        response = client.put(
            f"/api/goals/{django_goal.id}/move",
            json={"parent_id": career_goal.id}
        )
        assert response.status_code == 200
    ...
    def test_project_hierarchy_workflow(self, client: TestClient, db: Session):
        ...
        website_project = self.create_test_project(db, "Company Website", work_project.id)
