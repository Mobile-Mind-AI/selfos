...
class TestAdvancedChatScenarios:
    ...
    def test_context_switching_conversation(self, get_test_user_headers):
        ...
        for switch in topic_switches:
            ...
            has_context = any(
                word in content_lower for word in switch["expected_context"]
            )
            assert (
                has_context
            ), f"Missing context for {switch['topic']}: {switch['expected_context']}"
    ...
    def test_emotional_support_conversation(self, get_test_user_headers):
        ...
        for scenario in emotional_scenarios:
            ...
            content_lower = ai_response["content"].lower()
            has_appropriate_tone = any(
                phrase in content_lower for phrase in scenario["expected_tone"]
            )
            assert (
                has_appropriate_tone
            ), f"Inappropriate tone for {scenario['user_state']}: {ai_response['content']}"
    ...
    def test_habit_formation_guidance_conversation(self, get_test_user_headers):
        ...
        for i, message in enumerate(habit_formation_flow):
            ...
            has_habit_guidance = any(
                keyword in content_lower for keyword in expected_keywords
            )
            assert (
                has_habit_guidance
            ), f"Missing habit guidance at stage {i+1}: {expected_keywords}"
    ...
    def test_goal_modification_conversation(self, get_test_user_headers):
        ...
        for step in goal_modification_flow:
            ...
            content_lower = ai_response["content"].lower()
            has_appropriate_guidance = any(
                phrase in content_lower for phrase in step["expected"]
            )
            assert (
                has_appropriate_guidance
            ), f"Inappropriate guidance for {step['stage']}: {step['expected']}"
...
class TestChatStressTesting:
    ...
    def test_rapid_fire_chat_requests(self, get_test_user_headers):
        ...
        print(
            f"Processed {len(rapid_messages)} rapid requests in {total_time:.2f} seconds"
        )
    ...
    def test_very_long_message_handling(self, get_test_user_headers):
        ...
        """
            * 2
        )  # Double it to make it really long
        ...
        assert any(
            ...
            for word in ["complex", "many", "prioritize", "step", "overwhelming"]
        )
        ...
        for i in range(50):
            conversation_history.extend(
                [
                    {
                        "role": "user",
                        "content": f"User message {i+1} about my ongoing goals and progress",
                    },
                    {
                        "role": "assistant",
                        "content": f"Assistant response {i+1} providing guidance and support",
                    },
                ]
            )
        ...
        for edge_input in edge_cases:
            ...
            if response.status_code == 200:
                ...
                if len(ai_response["content"]) == 0:
                    print(
                        f"INFO: Empty response for edge case input: {repr(edge_input[:50])}"
                    )
...
class TestChatIntegrationScenarios:
    ...
    def test_chat_to_goal_decomposition_workflow(self, get_test_user_headers):
        ...
        goal_response = client.post(
            "/api/ai/decompose-goal/", headers=headers, json=goal_request
        )
        ...
        follow_up_chat = {
            ...
            "conversation_history": [
                {
                    "role": "user",
                    "content": "I've been thinking about learning data science to advance my career",
                },
        ...
        follow_up_response = client.post(
            "/api/ai/chat/", headers=headers, json=follow_up_chat
        )
        ...
        assert any(
            word in content_lower
            for word in ["motivation", "progress", "milestones", "celebrate"]
        )
    ...
    def test_chat_with_memory_retrieval(self, get_test_user_headers):
        ...
        assert any(
            word in content_lower
            for word in ["meditation", "challenge", "wellness", "another"]
        )
...
class TestChatMonitoring:
    ...
    def test_chat_response_times(self, get_test_user_headers):
        ...
        for message in test_messages:
            ...
            print(
                f"Message length: {len(message)}, Response time: {response_time:.2f}s"
            )
