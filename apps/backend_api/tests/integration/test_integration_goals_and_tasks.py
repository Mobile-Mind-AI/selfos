...
class TestGoalsAndTasksIntegration:
    ...
    def test_complete_workflow_goal_to_tasks(self):
        ...
        task3_update_response = client.put(
            f"/api/tasks/{task3['id']}", json=task3_update
        )
    ...
    def test_cascade_operations(self):
        ...
        remaining_goal_tasks = [
            t for t in tasks_after_delete if t["goal_id"] == goal_id
        ]
    ...
    def test_data_consistency(self):
        ...
        # Create goal
        goal_data = {"title": "Consistency Test", "status": "todo", "progress": 0.0}
        goal_response = client.post("/api/goals/", json=goal_data)
        goal_id = goal_response.json()["id"]
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if get_db in app.dependency_overrides:
        if app.dependency_overrides[get_db] == override_get_db_integration:
