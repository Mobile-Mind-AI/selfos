...
from models import AssistantProfile, Base, ConversationLog, ConversationSession
...
class TestAssistantConversationIntegration:
    ...
    def teardown_method(self):
        ...
        # Don't clear all overrides, just remove test-specific ones if needed
        pass

    @patch("services.intent_service.ConversationFlowManager.process_message")
    def test_conversation_with_specific_assistant(
    ...
    def test_conversation_with_default_assistant(self, mock_process_message, mock_user):
        ...
        mock_process_message.return_value = {
            ...
            "next_actions": [{"type": "continue_conversation", "context": {}}],
            "requires_clarification": False,
        }
        ...
        message_data = {"message": "Hello, how are you today?", "include_context": True}
    ...
    def test_conversation_without_assistant_profiles(self, mock_user):
        ...
        message_data = {"message": "I need help with my goals", "include_context": True}

        with patch(
            "services.intent_service.ConversationFlowManager.process_message"
        ) as mock_process:
    ...
    def test_multiple_assistants_different_responses(self, mock_user):
        ...
        with patch(
            "services.intent_service.ConversationFlowManager.process_message",
            side_effect=mock_process_with_assistant,
        ):
            ...
            formal_response = client.post(
                "/api/conversation/message/", json=formal_message
            )
            ...
            casual_response = client.post(
                "/api/conversation/message/", json=casual_message
            )
            ...
            assert (
                formal_data["intent_result"]["entities"]["title"] == "exercise regimen"
            )
    ...
    def test_assistant_temperature_settings_passed_through(self, mock_user):
        ...
        create_response = client.post(
            "/api/assistant_profiles/", json=high_temp_assistant
        )
        assistant_id = create_response.json()["id"]
