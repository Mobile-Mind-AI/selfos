...
class TestAnalyticsAPI:
    ...
    def test_full_preferences_update_and_analytics_flow(self, client, mock_user, db_session):
        ...
        notifications_changes = [h for h in history if h["preference_name"] == "notifications_enabled"]
    ...
    def test_get_preferences_summary_with_different_time_windows(self, client, mock_user, db_session):
    ...
    def test_analytics_endpoints_return_valid_response_structure(self, client):
        ...
        assert "preferences_changed" in summary
        assert "change_counts" in summary
        assert "most_changed_preference" in summary or summary["most_changed_preference"] is None
        assert "latest_change" in summary or summary["latest_change"] is None

    def test_preferences_update_creates_correct_history(self, client, mock_user, db_session):
