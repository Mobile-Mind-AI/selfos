...
class TestAIIntegration:
    ...
    def test_goal_decomposition_basic(self, get_test_user_headers):
        ...
        if data["model_used"] == "mock-model":
            assert (
                "research" in data["content"].lower()
                or "plan" in data["content"].lower()
            )
    ...
    def test_chat_goal_intent(self, get_test_user_headers):
        ...
        assert (
            data["intent_detected"] == "goal_setting" or data["intent_detected"] is None
        )
    ...
    def test_chat_with_history(self, get_test_user_headers):
        ...
        chat_request = {
            ...
            "conversation_history": [
                ...
                {
                    "role": "assistant",
                    "content": "Great! Let's create a fitness plan for you.",
                },
            ],
            "user_context": {},
        }
    ...
    def test_memory_search(self, get_test_user_headers):
        ...
        # Now search for it
        search_params = {"query": "books reading", "limit": 5}

        response = client.post(
            "/api/ai/memory/search", headers=headers, params=search_params
        )
    ...
    def test_error_handling_invalid_chat(self, get_test_user_headers):
        ...
        chat_request = {"message": "", "conversation_history": []}  # Empty message
...
class TestAIWorkflow:
    ...
    def test_goal_to_chat_workflow(self, get_test_user_headers):
        ...
        goal_request = {
            ...
            "life_areas": [
                {"id": 1, "name": "Health", "description": "Physical and mental health"}
            ],
            "existing_goals": [],
        }
    ...
    def test_multiple_goals_workflow(self, get_test_user_headers):
        ...
        goals = [
            {
                "goal_description": "Learn a new programming language",
                "context": "career development",
            },
            {
                "goal_description": "Build a personal project",
                "context": "apply new skills",
            },
            {
                "goal_description": "Contribute to open source",
                "context": "give back to community",
            },
        ]
        ...
        for goal_info in goals:
            goal_request = {
                ...
                "life_areas": [
                    {"id": 1, "name": "Career", "description": "Professional growth"}
                ],
                "existing_goals": [
                    {"title": g["goal_description"], "status": "planned"}
                    for g in goals[: len(responses)]
                ],
