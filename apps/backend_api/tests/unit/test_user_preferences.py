...
def pytest_sessionfinish(session, exitstatus):
    ...
    if get_db in app.dependency_overrides:
        if app.dependency_overrides[get_db] == override_get_db_user_preferences:
            del app.dependency_overrides[get_db]
    if get_current_user in app.dependency_overrides:
        if (
            app.dependency_overrides[get_current_user]
            == override_get_current_user_user_preferences
        ):
...
def test_update_preferences_creates_if_not_exist():
    ...
    update_data = {"tone": "coach", "default_view": "list"}
...
def test_quick_setup_preferences():
    ...
    response = client.post(
        ...
        params={"tone": "coach", "notifications": False, "default_view": "timeline"},
    )
...
def test_quick_setup_invalid_tone():
    ...
    response = client.post(
        "/api/user-preferences/quick-setup", params={"tone": "invalid_tone"}
    )
...
def test_quick_setup_invalid_view():
    ...
    response = client.post(
        "/api/user-preferences/quick-setup",
        params={"tone": "friendly", "default_view": "invalid_view"},
    )
...
def test_quick_setup_updates_existing():
    ...
    response = client.post(
        "/api/user-preferences/quick-setup",
        params={"tone": "professional", "notifications": False, "default_view": "list"},
    )
...
def test_preferences_validation():
    ...
    # Test invalid tone
    response = client.post("/api/user-preferences", json={"tone": "invalid_tone"})
    assert response.status_code == 422

    # Test invalid view mode
    response = client.post(
        "/api/user-preferences", json={"default_view": "invalid_view"}
    )
    assert response.status_code == 422
...
def test_preferences_timestamps():
    ...
    # Create preferences
    response = client.post("/api/user-preferences", json={"tone": "friendly"})
    ...
    update_response = client.put("/api/user-preferences", json={"tone": "coach"})
