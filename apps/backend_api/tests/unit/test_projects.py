...
class TestProjectCRUD:
    ...
    def test_list_projects_with_filters(
        self, client, db_session, test_user, test_life_area
    ):
...
class TestProjectProgress:
    ...
    def test_project_progress_with_goals_and_tasks(
        self, client, db_session, test_user, test_project
    ):
...
class TestProjectTimeline:
    ...
    def test_project_timeline_with_events(
        self, client, db_session, test_user, test_project
    ):
...
class TestProjectRelationships:
    ...
    def test_project_deletion_cascades(
        self, client, db_session, test_user, test_project
    ):
...
class TestProjectSecurity:
    ...
    def test_user_cannot_access_other_users_projects(self, client, db_session):
        ...
        response = client.put(
            f"/api/projects/{other_project.id}", json={"title": "Hacked"}
        )
        assert response.status_code == 404
