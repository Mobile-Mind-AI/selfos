...
# Use the shared test setup from conftest.py
pytestmark = pytest.mark.asyncio


@pytest.fixture
def client(isolated_test_setup):
    """Test client using shared database setup from conftest.py"""
...
@patch("firebase_admin.auth.create_user")
def test_register_failure(mock_create_user, client):
    ...
    response = client.post(
        "/auth/register",
        json={"username": "existing123", "password": "testpassword123"},
    )
...
def test_login_success(mock_create_token, client):
    """Test successful login"""
    # Mock Firebase custom token creation
    mock_create_token.return_value = b"mock_custom_token"

    response = client.post(
        "/auth/login", json={"username": "test_uid_123", "password": "testpassword"}
    )
...
@patch("firebase_admin.auth.create_custom_token")
def test_login_failure(mock_create_token, client):
    """Test login failure with invalid credentials"""
    # Mock Firebase auth failure
    mock_create_token.side_effect = Exception("Invalid user ID")

    response = client.post(
        "/auth/login", json={"username": "invalid_uid", "password": "wrongpassword"}
    )
...
def test_me_endpoint_success(mock_verify_token, client):
    ...
    response = client.get(
        "/auth/me", headers={"Authorization": "Bearer valid_firebase_token"}
    )
...
@patch("firebase_admin.auth.verify_id_token")
def test_me_endpoint_invalid_token(mock_verify_token, client):
...
@patch("firebase_admin.auth.create_user")
@patch("firebase_admin.auth.create_custom_token")
def test_social_login_google_success(
    mock_get_user, mock_create_user, mock_create_token, client
):
    ...
    data = response.json()
    assert "access_token" in data
    # Token will be a JWT, not the mock token
    assert "access_token" in data
    assert len(data["access_token"]) > 20  # JWT is longer
    assert data["token_type"] == "bearer"
    assert "user" in data
...
@patch("firebase_admin.auth.create_user")
@patch("firebase_admin.auth.create_custom_token")
def test_social_login_apple_success(
    mock_get_user, mock_create_user, mock_create_token, client
):
...
def test_social_login_missing_token(client):
...
def test_forgot_password_success(
    mock_send_email, mock_get_user, mock_generate_reset_link, client
):
...
@patch("firebase_admin.auth.generate_password_reset_link")
def test_forgot_password_user_not_found(mock_generate_reset_link, client):
    """Test password reset request for non-existent user"""
    # Mock Firebase user not found error
    mock_generate_reset_link.side_effect = firebase_admin.auth.UserNotFoundError(
        "User not found"
    )
