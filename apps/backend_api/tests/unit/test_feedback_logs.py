...
def pytest_sessionfinish(session, exitstatus):
    ...
    if get_db in app.dependency_overrides:
        if app.dependency_overrides[get_db] == override_get_db_feedback_logs:
            del app.dependency_overrides[get_db]
    if get_current_user in app.dependency_overrides:
        if (
            app.dependency_overrides[get_current_user]
            == override_get_current_user_feedback_logs
        ):
...
def test_create_minimal_feedback_log():
    ...
    feedback_data = {"context_type": "ui_interaction", "feedback_type": "neutral"}
...
def test_delete_feedback_log():
    ...
    feedback_data = {"context_type": "test", "feedback_type": "negative"}
...
def test_bulk_create_limit():
    ...
    bulk_data = [
        {"context_type": "test", "feedback_type": "neutral"} for _ in range(101)
    ]
...
def test_delete_bulk_feedback_logs():
    ...
    response = client.post(
        "/api/feedback-logs/bulk-delete/", json={"feedback_ids": created_ids}
    )
...
def test_bulk_delete_limit():
    ...
    response = client.post(
        "/api/feedback-logs/bulk-delete/", json={"feedback_ids": fake_ids}
    )
...
def test_bulk_delete_nonexistent_ids():
    ...
    response = client.post(
        "/api/feedback-logs/bulk-delete/",
        json={"feedback_ids": ["nonexistent1", "nonexistent2"]},
    )
