...
class TestBasicPermissions:
    ...
    @pytest.mark.asyncio
    async def test_owner_permissions(self, db_session: Session):
        ...
        for level in PermissionLevel:
            has_permission = await PermissionService.check_permission(
                user.uid, assistant.id, level, db_session
            )
            assert has_permission, f"Owner should have {level} permission"

    @pytest.mark.asyncio
    async def test_non_owner_no_permissions(self, db_session: Session):
        ...
        for level in PermissionLevel:
            has_permission = await PermissionService.check_permission(
                non_owner.uid, assistant.id, level, db_session
            )
            assert not has_permission, f"Non-owner should not have {level} permission"

    @pytest.mark.asyncio
    async def test_public_assistant_read_access(self, db_session: Session):
        ...
        has_read = await PermissionService.check_permission(
            non_owner.uid, assistant.id, PermissionLevel.READ, db_session
        )
        ...
        has_edit = await PermissionService.check_permission(
            non_owner.uid, assistant.id, PermissionLevel.EDIT, db_session
        )
        assert not has_edit, "Should not have edit access to public assistant"

    @pytest.mark.asyncio
    async def test_sharing_workflow(self, db_session: Session):
        ...
        has_permission = await PermissionService.check_permission(
            grantee.uid, assistant.id, PermissionLevel.READ, db_session
        )
        ...
        has_edit = await PermissionService.check_permission(
            grantee.uid, assistant.id, PermissionLevel.EDIT, db_session
        )
        has_admin = await PermissionService.check_permission(
            grantee.uid, assistant.id, PermissionLevel.ADMIN, db_session
        )
        ...
        has_permission_after = await PermissionService.check_permission(
            grantee.uid, assistant.id, PermissionLevel.READ, db_session
        )
        assert not has_permission_after, "Permission should be revoked"

    @pytest.mark.asyncio
    async def test_get_user_assistants(self, db_session: Session):
        ...
        db_session.add_all([owned_assistant, public_assistant])
        db_session.commit()
        
        # Get user1's assistants
        user1_assistants = await PermissionService.get_user_assistants(
            user1.uid, db_session
        )
    ...
    @pytest.mark.asyncio
    async def test_permission_hierarchy(self, db_session: Session):
        ...
        has_owner = await PermissionService.check_permission(
            grantee.uid, assistant.id, PermissionLevel.OWNER, db_session
        )
...
class TestVersioning:
    ...
    def test_version_update(self, db_session: Session):
