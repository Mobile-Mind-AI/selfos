...
from models import AssistantProfile, Base
...
def mock_user_2():
    ...
    return {"uid": "test_user_456", "email": "testuser2@example.com", "roles": ["user"]}
...
class TestAssistantProfilesCRUD:
    ...
    def test_create_assistant_profile(self, mock_user):
        ...
        assert (
            ...
            == SAMPLE_PROFILE_DATA["requires_confirmation"]
        )
        assert data["style"] == SAMPLE_PROFILE_DATA["style"]
        assert (
            data["dialogue_temperature"] == SAMPLE_PROFILE_DATA["dialogue_temperature"]
        )
    ...
    def test_update_assistant_profile(self, mock_user):
        ...
        response = client.patch(
            f"/api/assistant_profiles/{profile_id}", json=update_data
        )
...
class TestAssistantProfilesPersonality:
    ...
    def test_personality_preview(self, mock_user):
        ...
        assert (
            "playful" in summary["humor"].lower()
            or "humorous" in summary["humor"].lower()
        )
        assert (
            "empathetic" in summary["empathy"].lower()
            or "warm" in summary["empathy"].lower()
        )
        assert (
            "energetic" in summary["motivation"].lower()
            or "motivational" in summary["motivation"].lower()
        )
    ...
    def test_get_assistant_config(self):
        ...
        assert (
            "gpt-3.5-turbo" in data["supported_models"]
            or "gpt-4" in data["supported_models"]
        )
