...
class TestPermissionService:
    ...
    async def test_public_assistant_read_access(self, db_session: Session, sample_assistant, sample_users):
        ...
        # Get the assistant from current session and make it public
        assistant_id = sample_assistant.id
        assistant = db_session.query(AssistantProfile).filter(
        ...
        has_read = await PermissionService.check_permission(
            non_owner_id, assistant_id, PermissionLevel.READ, db_session
        )
        assert has_read, "Should have read access to public assistant"

        # Should not have write access
        has_edit = await PermissionService.check_permission(
            non_owner_id, assistant_id, PermissionLevel.EDIT, db_session
        )
        assert not has_edit, "Should not have edit access to public assistant"

    @pytest.mark.asyncio
    async def test_permission_hierarchy(self, db_session: Session, sample_assistant, sample_users):
        ...
        has_read = await PermissionService.check_permission(
            grantee_id, assistant_id, PermissionLevel.READ, db_session
        )
        assert has_read, "EDIT permission should include READ"

        # Should have EDIT permission
        has_edit = await PermissionService.check_permission(
            grantee_id, assistant_id, PermissionLevel.EDIT, db_session
        )
        ...
        has_admin = await PermissionService.check_permission(
            grantee_id, assistant_id, PermissionLevel.ADMIN, db_session
        )
        assert not has_admin, "EDIT permission should not include ADMIN"

    @pytest.mark.asyncio
    async def test_expired_permissions_ignored(self, db_session: Session, sample_assistant, sample_users):
    ...
    @pytest.mark.asyncio
    async def test_share_assistant_requires_admin(self, db_session: Session, sample_assistant, sample_users):
    ...
    @pytest.mark.asyncio
    async def test_cannot_grant_higher_permission(self, db_session: Session, sample_assistant, sample_users):
    ...
    @pytest.mark.asyncio
    async def test_revoke_permission(self, db_session: Session, sample_assistant, sample_users):
    ...
    @pytest.mark.asyncio
    async def test_get_user_assistants(self, db_session: Session, sample_users):
        ...
        db_session.add_all([owned_assistant, public_assistant, shared_assistant])
        db_session.commit()
        db_session.refresh(owned_assistant)
        db_session.refresh(public_assistant)
        db_session.refresh(shared_assistant)
        ...
        user1_assistants = await PermissionService.get_user_assistants(user1_id, db_session)
    ...
    @pytest.mark.asyncio
    async def test_update_existing_permission(self, db_session: Session, sample_assistant, sample_users):
        ...
        permissions = db_session.query(AssistantPermission).filter(
    ...
    @pytest.mark.asyncio
    async def test_get_user_permission_level(self, db_session: Session, sample_assistant, sample_users):
        ...
        owner_level = await PermissionService.get_user_permission_level(
            owner_id, assistant_id, db_session
        )
        ...
        grantee_level = await PermissionService.get_user_permission_level(
            grantee_id, assistant_id, db_session
        )
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_permissions(self, db_session: Session, sample_assistant, sample_users):
        ...
        all_permissions = db_session.query(AssistantPermission).filter(
            AssistantPermission.assistant_id == assistant_id
        ).all()
        assert len(all_permissions) == 2

        # Run cleanup
        cleaned_count = await PermissionService.cleanup_expired_permissions(db_session)
        assert cleaned_count == 1, "Should clean up 1 expired permission"

        # Verify only non-expired permission remains
        remaining_permissions = db_session.query(AssistantPermission).filter(
            AssistantPermission.assistant_id == assistant_id
        ).all()
    ...
    @pytest.mark.asyncio
    async def test_nonexistent_assistant(self, db_session: Session, sample_users):
        """Test permission check on non-existent assistant."""
        user_id = sample_users[0].uid
        fake_assistant_id = "non-existent-id"

        has_permission = await PermissionService.check_permission(
            user_id, fake_assistant_id, PermissionLevel.READ, db_session
        )
...
class TestAssistantVersioning:
    ...
    @pytest.fixture
    def sample_user(self, isolated_test_setup):
    ...
    @pytest.fixture
    def sample_assistant(self, isolated_test_setup, sample_user):
    ...
    def test_version_update(self, db_session: Session, sample_assistant):
    ...
    def test_version_is_timestamp(self, db_session: Session, sample_assistant):
