...
class FileValidator:
    ...
    async def validate_upload(
        ...
        if not file_type:
            raise HTTPException(
                status_code=400, detail=f"Unsupported file extension: {file_extension}"
            )
    ...
    async def _validate_file_content(
        cls, file_type: str, content: bytes, filename: str
    ) -> dict:
    ...
    def _validate_video_content(cls, content: bytes) -> dict:
        ...
        metadata = {"validated": True, "note": "Basic validation performed"}
    ...
    def _validate_document_content(cls, content: bytes, filename: str) -> dict:
        ...
        metadata = {"validated": True, "filename": filename}
        ...
        if filename.lower().endswith(".pdf"):
            ...
            if not (
                content.startswith(b"PK") or content.startswith(b"\xd0\xcf\x11\xe0")
            ):
