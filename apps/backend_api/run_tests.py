...
def run_module_by_module():
    ...
    for module in test_modules:
        ...
        summary_match = re.search(
            r"(\d+) failed.*?(\d+) passed|(\d+) passed.*?(\d+) failed|(\d+) passed",
            output,
        )
        if summary_match:
...
def main():
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Run all tests using module-by-module approach (default)",
    )
    parser.add_argument(
        "--all-together",
        action="store_true",
        help="Run all tests together (may have isolation issues)",
    )
    ...
    parser.add_argument(
        "--life-areas", action="store_true", help="Run life areas tests only"
    )
    parser.add_argument(
        "--media", action="store_true", help="Run media attachments tests only"
    )
    parser.add_argument(
        "--preferences", action="store_true", help="Run user preferences tests only"
    )
    parser.add_argument(
        "--feedback", action="store_true", help="Run feedback logs tests only"
    )
    parser.add_argument(
        "--stories", action="store_true", help="Run story sessions tests only"
    )
    ...
    # Run the tests
    description = "Running " + (
        ", ".join(test_targets) if test_targets else "all tests"
    )
    success = run_command(cmd, description)
    ...
    print(
        "  python run_tests.py                 # All tests (module-by-module, recommended)"
    )
    print(
        "  python run_tests.py --all-together  # All tests together (may have isolation issues)"
    )
