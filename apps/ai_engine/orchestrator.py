...
What specific health improvements or skills are you most excited about exploring?"""
            ...
            elif "time" in user_message and (
                "hour" in user_message or "week" in user_message
            ):
...
Starting with just 5-10 minutes of daily meditation can make a significant difference. What aspect of wellness would you like to explore another approach to?"""
                ...
                word in user_message
                for word in ["piano", "music", "ukulele", "instrument"]
            ):
                if "ukulele" in user_message and (
                    "easier" in user_message or "instead" in user_message
                ):
...
This sounds like a much more realistic and achievable goal! What draws you to the ukulele specifically? Are you interested in a particular style of music?"""
                elif "piano" in user_message and (
                    "not making progress" in user_message
                    or "struggling" in user_message
                ):
...
class ResponseCache:
    ...
    async def clear_expired(self):
        ...
        async with self._lock:
            expired_keys = [
                key for key, entry in self.cache.items() if entry.is_expired(self.ttl)
            ]
...
class AIOrchestrator:
    ...
    def _initialize_clients(self):
        ...
        logger.debug(
            f"Starting client initialization. Available providers: {self.config.get_available_providers()}"
        )
        ...
        logger.debug(
            f"Client initialization complete. Final clients: {list(self.clients.keys())}"
        )

    async def decompose_goal(
        self, request: GoalDecompositionRequest
        ...
        try:
            ...
            ai_response = await self._generate_response(
                prompt=prompt, use_case="goal_decomposition", request_id=request_id
            )
    ...
    async def generate_tasks(
        self, request: TaskGenerationRequest
        ...
        try:
            ...
            if request.generation_type == "next_tasks":
                ...
                prompt = TaskGenerationPrompts.suggest_next_tasks_prompt(
                    context=context, completed_tasks=request.completed_tasks
                )
            else:
                raise ValueError(
                    f"Unsupported generation type: {request.generation_type}"
                )

            # Get AI response
            ai_response = await self._generate_response(
                prompt=prompt, use_case="task_generation", request_id=request_id
            )
    ...
    async def chat(self, request: ConversationRequest) -> ConversationResponse:
        ...
        try:
            ...
            if request.user_context:
                ...
                if "emotional_state" in request.user_context:
                    ...
                    context_parts.append(
                        f"The user is feeling {emotional_state}. Respond with appropriate emotional support and understanding."
                    )
                ...
                if "goal_type" in request.user_context:
                    ...
                    if goal_type == "habit_formation":
                        context_parts.append(
                            "Focus on habit formation strategies, small steps, and building sustainable routines."
                        )
                    elif goal_type == "skill_learning":
                        context_parts.append(
                            "Provide learning guidance, practice strategies, and skill development advice."
                        )
                ...
                if "current_goal" in request.user_context:
                    ...
                    context_parts.append(
                        f"The user is working on a {goal} goal. Provide relevant advice and encouragement."
                    )

                if (
                    "struggling" in request.user_context
                    and request.user_context["struggling"]
                ):
                    context_parts.append(
                        "The user is struggling and needs motivational support. Be encouraging and provide practical solutions."
                    )

                if (
                    "support_needed" in request.user_context
                    and request.user_context["support_needed"]
                ):
                    context_parts.append(
                        "Provide supportive, understanding responses with actionable advice."
                    )

                if "complex_goal" in request.user_context:
                    context_parts.append(
                        "This is a complex, multi-faceted goal. Break it down into manageable steps and acknowledge the complexity."
                    )

                if "topic_switch" in request.user_context:
                    context_parts.append(
                        "The user is switching topics. Acknowledge the context switch and respond to the new topic appropriately."
                    )
                ...
                if "workflow_step" in request.user_context:
                    ...
                    if step == "clarification":
                        context_parts.append(
                            "The user is clarifying their goal. Ask about specific aspects like equipment, skills needed, business considerations, or practical details."
                        )
                    elif step == "constraints":
                        context_parts.append(
                            "The user is discussing constraints (budget, time). Provide realistic advice that works within their limitations."
                        )
                    elif step == "next_steps":
                        context_parts.append(
                            "The user wants to know next steps. Provide specific, actionable first steps they can take immediately."
                        )

                # Business/career context
                if "business_planning" in request.user_context:
                    context_parts.append(
                        "This involves business or career planning. Discuss expertise, experience, client acquisition, time management, and gradual transition strategies."
                    )

                # Long message context
                if (
                    "long_message" in request.user_context
                    or "complex_situation" in request.user_context
                ):
                    context_parts.append(
                        "The user has shared a complex situation with many factors. Acknowledge the complexity and help prioritize the most important steps."
                    )
            ...
            full_prompt = (
                f"{system_prompt}{context_guidance}\n\nUser: {request.message}"
            )
            ...
            # Get AI response
            ai_response = await self._generate_response(
                prompt=full_prompt, use_case="conversation", request_id=request_id
            )
    ...
    async def _generate_response(
        ...
        for provider_to_try in providers_to_try:
            ...
            if provider_to_try.value not in self.clients:  # Use string value for lookup
                logger.warning(
                    f"Provider {provider_to_try.value} not in clients dictionary"
                )
                continue

            try:
                client = self.clients[
                    provider_to_try.value
                ]  # Use string value for lookup
                config = self.config.get_model_config(use_case, provider_to_try)

                logger.debug(
                    f"Sending prompt to {provider_to_try.value}: {prompt[:200]}..."
                )
                ...
                logger.debug(
                    f"{provider_to_try.value} response: {result.get('content', '')[:200]}..."
                )
        ...
        if last_error is None:
            raise Exception(
                f"No providers available. Providers to try: {providers_to_try}, Available clients: {list(self.clients.keys())}"
            )
        else:
