...
class ConversationToolsHandler(BaseToolsHandler):
    ...
    async def call_tool(
        ...
        try:
            if name == "conversation_process_message":
                ...
                return [
                    TextContent(type="text", text=f"Unknown conversation tool: {name}")
                ]
                    ...
                    text=json.dumps(
                        {
                            ...
                            "conversation_state": {
                                "turn_count": result["conversation_state"][
                                    "turn_count"
                                ],
                                "current_intent": result["conversation_state"][
                                    "current_intent"
                                ],
                                "incomplete_entities": result["conversation_state"][
                                    "incomplete_entities"
                                ],
                            },
                        },
    ...
    async def _handle_classify_intent(
        ...
        try:
            ...
            result = await classifier.classify_intent(
                arguments["message"], user_context
            )
            ...
            return [
                TextContent(type="text", text=f"Error classifying intent: {str(e)}")
            ]
    ...
    async def _handle_execute_intent(
        ...
        try:
            ...
            if intent == "create_goal":
                ...
                return await self._execute_update_settings(
                    user_id, entities, confidence
                )
                ...
                return await self._execute_chat_response(
    ...
    async def _handle_get_analytics(
        ...
        try:
            ...
            return [TextContent(type="text", text=json.dumps(analytics_data, indent=2))]
    ...
    async def _handle_get_sessions(
        ...
        try:
            ...
            return [TextContent(type="text", text=json.dumps(sessions_data, indent=2))]
    ...
    async def _handle_provide_feedback(
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error providing feedback: {str(e)}")
            ]
