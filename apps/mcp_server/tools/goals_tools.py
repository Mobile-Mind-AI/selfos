...
class GoalsToolsHandler(BaseToolsHandler):
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "priority": {
                            "type": "string",
                            "description": "Goal priority (low, medium, high)",
                        },
                    },
    ...
    async def _delete_goal(self, arguments: dict) -> dict:
        ...
        try:
            ...
            return self.format_success_response(
                {"deleted_goal_id": goal_id}, "goals_delete"
            )
    ...
    async def _search_goals(self, arguments: dict) -> dict:
        ...
        try:
            ...
            search_filter = or_(
                Goal.title.ilike(f"%{query}%"), Goal.description.ilike(f"%{query}%")
            )

            goals = (
                db.query(Goal)
                .filter(and_(Goal.user_id == user_id, search_filter))
                .limit(limit)
                .all()
            )
            ...
            return self.format_success_response(
                {"goals": goals_data, "query": query, "count": len(goals_data)},
                "goals_search",
            )
