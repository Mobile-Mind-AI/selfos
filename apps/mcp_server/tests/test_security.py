...
from security import ClientPermissions, MCPPermissions, PermissionLevel
...
class TestMCPPermissions:
    ...
    def test_default_permissions(self, permissions):
        ...
        read_only = permissions.get_client_permissions(
            "test_client_ro", PermissionLevel.READ_ONLY
        )
        ...
        read_write = permissions.get_client_permissions(
            "test_client_rw", PermissionLevel.READ_WRITE
        )
        assert read_write.level == PermissionLevel.READ_WRITE
        assert "goals_*" in read_write.allowed_tools

        # Test admin permissions
        admin = permissions.get_client_permissions(
            "test_client_admin", PermissionLevel.ADMIN
        )
    ...
    async def test_tool_permission_checking(self, permissions):
        ...
        permissions.set_client_permissions(
            "readonly_client", permissions.PERMISSION_CONFIGS[PermissionLevel.READ_ONLY]
        )
...
class TestMCPAuth:
    ...
    async def test_api_key_authentication(self, auth_provider):
        ...
        valid_credentials = {"type": "api_key", "key": "sk-selfos-user123-randomstring"}

        success, user_id, user_info = await auth_provider.authenticate_client(
            valid_credentials
        )
        ...
        invalid_credentials = {"type": "api_key", "key": "invalid-key-format"}
    ...
    async def test_unknown_auth_type(self, auth_provider):
        ...
        unknown_credentials = {"type": "unknown_type", "token": "some_token"}
    ...
    async def test_permission_validation(self, auth_provider):
        ...
        can_write = await auth_provider.validate_permissions(
            "user123", "goals", "create"
        )
        assert can_write is True

        can_delete = await auth_provider.validate_permissions(
            "user123", "goals", "delete"
        )
