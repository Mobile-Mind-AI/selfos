...
class MCPPermissions:
    ...
    PERMISSION_CONFIGS = {
        ...
        PermissionLevel.ADMIN: ClientPermissions(
            level=PermissionLevel.ADMIN, allowed_tools=["*"], allowed_resources=["*"]
        ),
    }
    ...
    def set_client_permissions(self, client_id: str, permissions: ClientPermissions):
    ...
    async def check_tool_permission(
        ...
        for rule in permissions.custom_rules:
            if fnmatch.fnmatch(tool_name, rule.pattern):
                if not rule.allowed:
                    logger.warning(
                        f"Tool {tool_name} denied by custom rule: {rule.description}"
                    )
                return rule.allowed
    ...
    async def check_resource_permission(
        self, user_id: str, client_id: str, resource_uri: str
    ) -> bool:
        """
        ...
        if self._matches_patterns(resource_type, permissions.denied_resources):
            logger.warning(
                f"Resource {resource_type} explicitly denied for client {client_id}"
            )
            return False
    ...
    def _extract_resource_type(self, resource_uri: str) -> str:
        ...
        if resource_uri.startswith("selfos://"):
            ...
            if len(path_parts) >= 2:
                ...
                resource_prefix = (
                    path_parts[0].rstrip("s")
                    if path_parts[0].endswith("s")
                    else path_parts[0]
                )
    ...
    async def validate_rate_limit(
        self, client_id: str, operation: str, window_seconds: int = 60
    ) -> bool:
        """
        ...
        client_limits[operation] = [
            timestamp
            for timestamp in client_limits[operation]
        ...
        if len(client_limits[operation]) >= max_requests:
            logger.warning(
                f"Rate limit exceeded for client {client_id}, operation {operation}"
            )
            return False
    ...
    def audit_log(
        ...
        if success:
            logger.info(
                f"AUDIT: {operation} on {resource} by {user_id} via {client_id}"
            )
        else:
            logger.warning(
